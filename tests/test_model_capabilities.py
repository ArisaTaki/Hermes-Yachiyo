...
from apps.shell.model_capabilities import (
    build_native_image_input_capability,
...
def test_xiaomi_pro_auto_uses_text_pipeline_despite_stale_models_cache(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "apps.shell.model_capabilities._load_models_dev_cache",
        lambda: {
            "xiaomi": {
                "models": {
                    "mimo-v2.5-pro": {
    ...
    capability = build_native_image_input_capability(
        provider="xiaomi",
        model="mimo-v2.5-pro",
...
def test_effective_provider_inference_maps_profile_provider_aliases():
    assert infer_effective_native_provider("xiaomi_mimo") == "xiaomi"
    assert infer_effective_native_provider("google_gemini") == "gemini"
    assert infer_effective_native_provider("moonshot") == "kimi-coding"
