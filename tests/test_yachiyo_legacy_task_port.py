...
def test_runtime_planner_covers_migrated_desktop_samples_before_cleanup() -> None:
    ...
    assert coverage["remaining_fallback_count"] == 8
    assert coverage["planner_covered_fallback_count"] == 8
    assert coverage["compatibility_cleanup_pending_count"] == 8
    assert {
...
def test_runtime_planner_reports_remaining_compatibility_fallback_contracts() -> None:
    ...
    assert len(fallback_contracts) == 8
