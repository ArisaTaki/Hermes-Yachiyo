...
def test_stream_smoke_handles_sse_event_split_across_response_chunks_without_leaking(monkeypatch):
    requests: list[dict] = []
    leaked_secret = "sk-stream-split-sse-secret123456"
    ...
    class FakeResponse:
        ...
        def __iter__(self):
            yield b'data: {"choices":[{"delta":{"content":"split '
            yield b'chunk "}}]}\n\n'
            yield (
                b'data: {"choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_split_sse",'
                b'"type":"function","function":{"name":"workspace_read",'
            )
            yield b'"arguments":"{\\"path\\":\\"README.md\\"}"}}]}}]}\n\n'
    ...
    assert summary["ok"] is True
    assert summary["chunk_count"] == 3
    assert summary["content_chars"] == len("split chunk ")
    assert summary["finish_reasons"] == ["tool_calls"]
    assert summary["tool_call_delta_count"] == 1
    assert summary["tool_call_count"] == 1
    assert summary["tool_calls"] == [
    ...
    assert leaked_secret not in summary_json
    assert "README.md" not in summary_json
    assert "split chunk" not in summary_json
...
def test_stream_smoke_accepts_responses_completed_top_level_finish_reason(monkeypatch):
    requests: list[dict] = []
    ...
    class FakeResponse:
        ...
        def __iter__(self):
            yield event({"type": "response.output_text.delta", "delta": "responses top-level finish"})
            yield event(
                {
                    "type": "response.completed",
                    "response": {
    ...
    summary = smoke.run_stream_smoke(
        base_url="https://api.example.test/v1",
        model="demo-responses-top-level-finish-model",
        api_key="sk-stream-smoke-secret123456",
        require_content=True,
        expect_finish_reasons=["stop"],
    )
    ...
    assert requests[0]["stream"] is True
    assert summary["ok"] is True
    assert summary["content_chars"] == len("responses top-level finish")
    assert summary["finish_reasons"] == ["stop"]
...
def test_stream_smoke_uses_responses_output_text_done_snapshot(monkeypatch):
    ...
    summary = smoke.run_stream_smoke(
        base_url="https://api.example.test/v1",
        model="demo-responses-output-text-done-model",
        api_key="sk-stream-smoke-secret123456",
        require_content=True,
        expect_finish_reasons=["stop"],
    )
...
def test_stream_smoke_uses_responses_output_text_done_list_snapshot(monkeypatch):
    requests: list[dict] = []
    ...
    summary = smoke.run_stream_smoke(
        base_url="https://api.example.test/v1",
        model="demo-responses-output-text-done-list-model",
        api_key="sk-stream-smoke-secret123456",
        require_content=True,
        expect_finish_reasons=["stop"],
    )
    ...
    assert summary["content_chars"] == len(expected)
    assert summary["finish_reasons"] == ["stop"]
    assert "draft value" not in summary_json
    assert "final Responses list" not in summary_json
    assert "snapshot" not in summary_json
...
def test_stream_smoke_uses_responses_reasoning_summary_done_snapshot(monkeypatch):
    requests: list[dict] = []
    expected_reasoning = "final Responses reasoning summary"
            ...
            yield event(
                {
                    ...
                    "output_index": 0,
                    "summary_index": 0,
                    "text": expected_reasoning,
                }
            )
            yield event({"type": "response.output_text.delta", "delta": "final answer"})
            yield event(
                {
                    "type": "response.completed",
                    "response": {
    ...
    summary = smoke.run_stream_smoke(
        base_url="https://api.example.test/v1",
        model="demo-responses-reasoning-summary-model",
        api_key="sk-stream-smoke-secret123456",
        require_content=True,
        require_reasoning=True,
        expect_finish_reasons=["stop"],
    ...
    assert summary["ok"] is True
    assert summary["content_chars"] == len("final answer")
    assert summary["reasoning_chars"] == len(expected_reasoning)
    assert summary["finish_reasons"] == ["stop"]
    assert expected_reasoning not in summary_json
    assert "draft reasoning summary" not in summary_json


def test_stream_smoke_uses_responses_reasoning_output_item_done_snapshot(monkeypatch):
    requests: list[dict] = []
    expected_part_reasoning = "draft reasoning item summary"
    expected_reasoning = "final Responses reasoning item summary"
            ...
            yield event(
                {
                    ...
                    "output_index": 0,
                    "summary_index": 1,
                    "part": {"type": "summary_text", "text": expected_part_reasoning},
                }
            )
            yield event(
                {
                    ...
                    "output_index": 0,
                    "item": {
                        "id": "rs_response_reasoning",
                        "type": "reasoning",
                        "summary": [{"type": "summary_text", "text": expected_reasoning}],
                    },
                }
            )
            yield event({"type": "response.output_text.delta", "delta": "final answer"})
    ...
    summary = smoke.run_stream_smoke(
        base_url="https://api.example.test/v1",
        model="demo-responses-reasoning-item-model",
        api_key="sk-stream-smoke-secret123456",
        require_content=True,
        require_reasoning=True,
        expect_finish_reasons=["stop"],
    ...
    assert summary["ok"] is True
    assert summary["content_chars"] == len("final answer")
    assert summary["reasoning_chars"] == len(expected_reasoning) + len(expected_part_reasoning)
    assert summary["finish_reasons"] == ["stop"]
    assert expected_reasoning not in summary_json
    assert expected_part_reasoning not in summary_json
