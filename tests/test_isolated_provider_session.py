...
def test_start_isolated_provider_session_accepts_request_provider_manifest(
    monkeypatch,
    tmp_path,
) -> None:
    for key in session_module._ENV_KEYS:
    ...
    manifest_path.write_text(
        json.dumps(
            {
                "provider_id": "request-manifest-virtual-desktop",
                "provider_kind": "sandbox_desktop",
                "supported_tools": ["app.open", "desktop.verify"],
                "keyboard_mouse_capture_supported": True,
                "foreground_mutation_supported": True,
    ...
    assert popen_calls[0]["command"] == [sys.executable, str(provider_script)]
    assert popen_calls[0]["cwd"] == str(tmp_path)
    assert popen_calls[0]["env"]["OHA_YACHIYO_DESKTOP_PROVIDER_REQUESTED_TOOLS"] == (
        "app.open,desktop.verify"
    )
    assert started["started"] is True
    assert started["source"] == "managed_external_provider_session"
    assert started["provider_id"] == "request-manifest-virtual-desktop"
    assert started["provider_manifest_evidence"]["provider_id"] == (
        "request-manifest-virtual-desktop"
    )
    assert started["desktop_session_kind"] == "virtual_desktop"
    assert started["desktop_backend_ready_for_public_release"] is True
    assert session_module.os.environ["OHA_YACHIYO_DESKTOP_PROVIDER_EXECUTE_URL"] == (
    ...
    assert "OHA_YACHIYO_DESKTOP_PROVIDER_MANIFEST" not in session_module.os.environ
...
def test_annotate_envelope_routes_isolated_preferred_app_open_through_running_provider() -> None:
    ...
    annotated = annotate_envelope_with_desktop_provider_session(envelope, session)
    request = annotated["requests"][0]

    assert request["desktop_provider_session"]["provider_id"] == "local-isolated-desktop"
    assert request["sandbox_provider"]["provider_kind"] == "sandbox_desktop"
    assert request["sandbox_provider"]["provider_id"] == "local-isolated-desktop"
    assert request["sandbox_provider"]["desktop_session_isolated"] is True
    assert request["sandbox_provider"]["foreground_takeover_required"] is False
    assert request["desktop_execution_route"]["selected_provider_kind"] == "sandbox_desktop"
    assert request["desktop_execution_route"]["selected_provider_id"] == (
        "local-isolated-desktop"
    )
    assert request["desktop_execution_route"]["status"] == "sandbox_ready"
    assert request["desktop_execution_route"]["provider_execution_required"] is True
    assert request["desktop_execution_route"]["foreground_takeover_required"] is False
