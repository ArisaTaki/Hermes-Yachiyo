...
def test_task_intent_router_covers_agent_work_domains() -> None:
    cases = (
        ...
        (
            ...
            "code_task",
            ["file.workspace_read", "file.workspace_write", "terminal.execution"],
        ),
        (
            "帮我写一个 Python 脚本处理日志",
            "code_task",
            ["file.workspace_read", "file.workspace_write"],
        ),
...
def test_runtime_planner_plans_code_changes_after_workspace_inspection() -> None:
    ...
    writable_decision = RuntimePlanner().decision(
        "帮我写一个 Python 脚本处理日志",
        allowed_tools=[
            "workspace.list",
            "workspace.read",
...
def test_runtime_planner_routes_code_test_diagnostics_to_approval_terminal_plan() -> None:
    ...
    assert decision.selected_intent.inputs == {
        ...
        "code_change_hint": {"mode": "fix"},
    }
    assert decision.selected_intent.expected_outputs == ["patch", "test_output"]
    assert decision.selected_intent.required_capabilities == [
        "file.workspace_read",
        "file.workspace_write",
        "terminal.execution",
    ...
    apply_step = _step_by_id(decision, "apply-code-changes")
    assert apply_step.tool_name == "workspace.write_patch"
    assert apply_step.input_preview == {
        "mode": "fix",
...
def test_runtime_planner_prefers_research_deliverable_over_browser_app_hint() -> None:
    ...
    plain_search_report = RuntimePlanner().decision(
        "搜索 open hanako 并输出报告",
        allowed_tools=["browser.open_url_and_extract_text", "artifact.write", "workspace.list"],
    )
    assert plain_search_report.selected_intent.kind == "web_research"
    assert plain_search_report.selected_intent.inputs == {
        "url_hint": "https://www.google.com/search?q=open+hanako",
        "browser_action": "open_url_extract",
        "query": "open hanako",
    }
    assert _step_by_id(plain_search_report, "extract-web-url-text").tool_name == (
        "browser.open_url_and_extract_text"
    )
...
def test_runtime_planner_inspects_app_before_app_scoped_ui_operation() -> None:
    ...
    creative_canvas = RuntimePlanner().decision(
    ...
    assert creative_canvas.selected_intent.kind == "desktop_operation"
    assert creative_canvas.selected_intent.inputs["app_name_hint"] == "Pixelmator Pro"
    assert creative_canvas.selected_intent.inputs["safe_shortcut_hint"] == {
        "action": "new_document"
    }
    assert creative_canvas.selected_intent.inputs["creative_canvas_hint"] == {
        "kind": "image_canvas",
        "width": 1024,
        "height": 1024,
    }
    assert [step.step_id for step in creative_canvas.plan.tool_plan.steps] == [
        "inspect-app",
        "operate-foreground-ui",
        "verify-desktop-result",
    ]
    assert _step_by_id(creative_canvas, "inspect-app").input_preview == {
    ...
    assert _step_by_id(creative_canvas, "operate-foreground-ui").input_preview == {
        "app_name": "Pixelmator Pro",
        "action": "new_document",
    }
    assert _step_by_id(creative_canvas, "verify-desktop-result").input_preview == {
        "role_filter": "text",
        "limit": 80,
    }
    assert planner_tool_requests(
...
def test_runtime_planner_routes_explicit_browser_url_open_actions() -> None:
    ...
    scrolled_screenshot = RuntimePlanner().decision(
        "打开 example.com 然后向下滚动两页并截图",
        allowed_tools=[*allowed, "desktop.safe_scroll", "screen.capture"],
    )

    assert scrolled_screenshot.selected_intent.inputs == {
        "url_hint": "https://example.com",
        "browser_action": "open_url_screenshot",
        "reason": "user asked to capture the browser page after opening a URL",
    }
    assert [step.step_id for step in scrolled_screenshot.plan.tool_plan.steps] == [
        "open-web-url",
        "scroll-opened-web-page",
        "verify-opened-web-page",
    ]
    assert _step_by_id(scrolled_screenshot, "scroll-opened-web-page").input_preview == {
        "direction": "down",
        "pages": 2,
    }
...
def test_runtime_planner_discovers_generic_communication_app_before_acting() -> None:
    allowed_tools = [
        "desktop.list_apps",
        "app.open",
        "desktop.active_window",
    ...
    cases = (
        ("打开聊天软件", "messaging", None),
        ("打开一个聊天软件，给 Alice 输入 hello", "messaging", "messaging"),
        ("open a messenger app", "messaging", None),
        ("open an email app", "mail", "mail"),
    )

    for prompt, query, capability_query in cases:
        decision = RuntimePlanner().decision(prompt, allowed_tools=allowed_tools)
...
def test_runtime_planner_routes_app_scoped_search_to_desktop_sequence() -> None:
    ...
    for prompt, expected_target in (
        ("在一个叫 SuperData Studio 的应用里搜索 sales.csv", "搜索"),
        ("in an app called SuperData Studio search sales.csv", "Search"),
        ("search sales.csv in an app called SuperData Studio", "Search"),
    ):
        named_file_search = RuntimePlanner().decision(
            prompt,
            allowed_tools=[
                "desktop.list_apps",
                "app.focus",
        ...
        assert named_file_search.selected_intent.kind == "desktop_operation"
        assert named_file_search.selected_intent.inputs["app_name_hint"] == "SuperData Studio"
        assert named_file_search.selected_intent.inputs["app_search_hint"] == {
            "query": "sales.csv",
            "target": expected_target,
        }
        assert _step_by_id(named_file_search, "type-app-search-query").input_preview == {
            "text": "sales.csv"
        }

    for prompt, app_control_tool, app_shortcut_tool in (
        (
        ...
        scoped_shortcut_search = RuntimePlanner().decision(
            prompt,
            allowed_tools=[
                "desktop.list_apps",
                app_control_tool,
        ...
        assert _step_by_id(scoped_shortcut_search, "focus-app-search-field").input_preview == {
            "app_name": "SuperData Studio",
            "action": "find",
        }
...
def test_runtime_planner_defaults_email_draft_channel_to_mail_without_send() -> None:
    allowed_tools = [
        "app.focus",
        "desktop.safe_shortcut",
        "desktop.safe_type_text",
    ...
    prompt = "帮我起草一封邮件给张三，内容是会议改到明天上午十点，不要发送"
...
def test_runtime_planner_routes_direct_context_communication_send_sequence() -> None:
    ...
    app_scoped_selection_tools = [
        "app.focus_and_safe_shortcut",
        "app.focus_and_safe_type_text",
        "desktop.search_submit",
        "desktop.submit_foreground",
    ]
    app_scoped_selection = RuntimePlanner().decision(
        "send selected text in Slack to yachiyo",
        allowed_tools=app_scoped_selection_tools,
    )

    assert app_scoped_selection.plan.tool_plan.missing_capabilities == []
    assert _step_by_id(
        app_scoped_selection,
        "copy-communication-body-source",
    ).tool_name == "app.focus_and_safe_shortcut"
    assert _step_by_id(
        app_scoped_selection,
        "copy-communication-body-source",
    ).input_preview == {"app_name": "Slack", "action": "copy"}
    assert _step_by_id(
        app_scoped_selection,
        "paste-communication-message",
    ).input_preview == {"app_name": "Slack", "action": "paste"}
    assert planner_tool_requests(
    ...
    ) == [
        {
            "protocol": "json_fallback",
            "tool": "app.focus_and_safe_shortcut",
            "input": {"app_name": "Slack", "action": "copy"},
            "source": "runtime_planner",
            "planning_reason": "planner_fallback_communication_send",
        },
        {
...
def test_runtime_planner_can_fall_back_to_artifact_for_communication_draft() -> None:
    ...
    assert [step.step_id for step in desktop_tools_draft.plan.tool_plan.steps] == [
        "open-or-focus-app",
        "focus-communication-recipient-search",
        "type-communication-recipient",
        "submit-communication-recipient-search",
    ...
    assert _step_by_id(desktop_tools_draft, "open-or-focus-app").input_preview == {
        "app_name": "Mail"
    }
    assert _step_by_id(desktop_tools_draft, "draft-communication-message").input_preview == {
        "text": "说明会议延期"
    }
    assert planner_tool_requests(
        "给 Alice 写一封邮件说明会议延期",
...
def test_runtime_planner_routes_open_generic_chat_app_with_visible_summary() -> None:
    prompt = "打开一个聊天应用给 Alice 发送当前窗口内容摘要"
    allowed_tools = [
        "desktop.ui_elements",
        "desktop.list_apps",
        "app.open_and_safe_shortcut",
...
def test_planner_desktop_tool_requests_maps_app_search_content_artifact() -> None:
    ...
    named_app_file_search_requests = planner_desktop_tool_requests(
        "在一个叫 SuperData Studio 的应用里搜索 sales.csv",
        allowed_tools=[
            "desktop.list_apps",
            "app.focus",
    ...
    assert named_app_file_search_requests == [
...
def test_planner_desktop_tool_requests_discovers_generic_media_app_when_query_playback_tool_is_missing() -> None:
    requests = planner_desktop_tool_requests(
        "打开一个可用的音乐应用播放超时空辉夜姬",
        allowed_tools=[
            "desktop.list_apps",
...
def test_planner_tool_requests_maps_explicit_browser_url_plan() -> None:
    ...
    scrolled_screenshot_requests = planner_desktop_tool_requests(
        "打开 example.com 然后向下滚动两页并截图",
        allowed_tools=[
            "browser.open_url",
            "browser.open_url_and_screenshot",
    ...
    assert scrolled_screenshot_requests == [
        {
            "protocol": "json_fallback",
            "tool": "browser.open_url",
            "input": {"url": "https://example.com"},
