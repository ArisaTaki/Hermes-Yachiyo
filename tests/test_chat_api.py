...
def test_send_message_executes_direct_data_analysis_task_and_records_artifact(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct data analysis task should not call model")
        ),
    )
    try:
        ...
        assert result["status"] == "completed"
        assert result["agent_task"]["status"] == "completed"
        assert tool_call["tool_name"] == "data.analyze"
...
def test_send_message_executes_direct_music_control_task(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    system_control_calls: list[str] = []
    apple_control_calls: list[str] = []
    monkeypatch.setattr(
        "apps.shell.agent_runtime.get_model_profile_service",
        lambda: SimpleNamespace(
            get_defaults=lambda: {"chat": ""},
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct music control task should not call model")
        ),
    )

    def fake_system_media_control(action: str) -> dict:
        system_control_calls.append(action)
        return {
            "ok": True,
    ...
    try:
        ...
        assert apple["status"] == "completed"
        assert apple["agent_task"]["status"] == "completed"
        assert apple["agent_task"]["summary"] == "已暂停 Apple Music。当前：超时空辉夜姬 - Yachiyo。"
        assert apple["agent_task"]["tool_calls"][-1]["tool_name"] == "media.apple_music_control"
        assert apple_task is not None
        assert apple_task.status == TaskStatus.COMPLETED
        assert apple_user.metadata["daily_desktop_tool"] == "media.apple_music_control"
        assert apple_control_calls == ["pause"]
        assert apple_run["status"] == "completed"
        assert "agent.desktop.intent_planned" in apple_event_types
        assert "agent.tool.call" in apple_event_types
        assert "agent.desktop.intent_completed" in apple_event_types
        assert "model.request.started" not in apple_event_types
        assert "model.requested" not in apple_event_types
    finally:
        service.close()
        store.close()


def test_send_message_executes_apple_music_status_without_model(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    status_calls = 0
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("music status task should not call model")
        ),
    )
    monkeypatch.setattr(
        "apps.shell.agent.tools.desktop.apple_music_play",
        lambda query: (_ for _ in ()).throw(
            AssertionError(f"music status task should not play query: {query}")
        ),
    )

    def fake_apple_music_status() -> dict:
    ...
    monkeypatch.setattr(
        "apps.shell.agent.tools.desktop.apple_music_status",
        fake_apple_music_status,
    )
    try:
        result = api.send_message("现在播放什么歌")
        task = runtime.state.get_task(result["task_id"])
        run = service.get_run(result["run_id"])
        event_types = [
            event["event_type"]
        ...
        assert result["status"] == "completed"
        assert result["agent_task"]["status"] == "completed"
        assert result["agent_task"]["summary"] == "Apple Music 当前播放：超时空辉夜姬 - Yachiyo。"
        assert result["agent_task"]["tool_calls"][-1]["tool_name"] == "media.apple_music_status"
        assert task is not None
        assert task.status == TaskStatus.COMPLETED
        assert task.result == "Apple Music 当前播放：超时空辉夜姬 - Yachiyo。"
        assert user.metadata["daily_desktop_tool"] == "media.apple_music_status"
...
def test_send_message_executes_main_chat_runnable_app_open_without_model(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
    ...
    try:
        ...
        assert user.metadata["runnable_kind"] == "main"
        assert user.metadata["daily_desktop_intent"] is True
        assert user.metadata["daily_desktop_tool"] == "app.open"
        assert user.metadata["daily_desktop_tools"] == ["app.open"]
...
def test_send_message_opens_explicit_desktop_client_without_model(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("explicit desktop client open should not call model")
        ),
    )
...
def test_send_message_executes_app_open_new_document_without_model(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    calls: list[tuple[str, str]] = []
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("app open new document task should not call model")
        ),
    )
...
def test_send_message_executes_app_open_new_item_without_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("app open new event task should not call model")
        ),
    )
...
def test_send_message_executes_app_open_browser_back_without_fake_app_name(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("app open browser back task should not call model")
        ),
    )
...
def test_send_message_executes_browser_shortcut_search_sequence_without_model(
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("browser shortcut search sequence should not call model")
        ),
    )
    ...
    try:
        ...
        assert calls == [
            ...
            ("focus", "Google Chrome"),
            ("shortcut", "new_tab"),
            ("open_url", "https://www.google.com/search?q=OpenAI"),
        ]
        assert result["agent_task"]["status"] == "completed"
        assert result["agent_task"]["needs_user_action"] is False
        assert result["agent_task"]["pending_approvals"] == []
        assert any(
            call["tool_name"] == "app.open_and_safe_shortcut"
            and call["input_preview"] == {"app_name": "Google Chrome", "action": "new_tab"}
...
def test_send_message_executes_app_prefix_find_shortcut_without_model(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    calls: list[tuple[str, str]] = []
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("app scoped find shortcut task should not call model")
        ),
    )
...
def test_send_message_executes_direct_daily_desktop_music_play_task(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct desktop music play task should not call model")
        ),
    )
...
def test_send_message_summarizes_apple_music_search_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct desktop music search fallback should not call model")
        ),
    )
    ...
    def fake_apple_music_play(query: str) -> dict:
        ...
        return {
            ...
            "fallback_result": {
                ...
                "data": {
                    "query": query,
                    "url": "https://music.apple.com/search?term=chrono",
...
def test_send_message_executes_direct_app_open_task(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    open_calls: list[str] = []
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct app open task should not call model")
        ),
    )
...
def test_send_message_opens_named_music_app_and_attempts_playback_without_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("named music app open task should not call model")
        ),
    )
...
def test_send_message_executes_common_folder_with_open_path(tmp_path, monkeypatch):
    ...
    try:
        ...
        for prompt, expected_path in (
            ...
            assert result["ok"] is True
            assert result["status"] == "completed"
            assert result["agent_task"]["summary"] == f"已打开文件夹：{expected_path}。"
            assert result["agent_task"]["tool_calls"][-1]["tool_name"] == "desktop.open_path"
            assert task is not None
            assert task.status == TaskStatus.COMPLETED
            assert task.result == f"已打开文件夹：{expected_path}。"
            assert open_path_calls[-1] == expected_path
            assert reveal_calls == []
            assert app_open_calls == []
            assert run["status"] == "completed"
        ...
        assert result["ok"] is True
        assert result["status"] == "completed"
        assert result["agent_task"]["summary"] == "已打开文件夹：/tmp。"
        assert result["agent_task"]["tool_calls"][-1]["tool_name"] == "desktop.open_path"
        assert task is not None
        assert task.status == TaskStatus.COMPLETED
        assert task.result == "已打开文件夹：/tmp。"
        assert open_path_calls[-1] == "/tmp"
        assert reveal_calls == []
        assert app_open_calls == []
        assert run["status"] == "completed"
...
def test_send_message_executes_latest_download_open_path_without_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct latest-download task should not call model")
        ),
    )
    ...
    try:
        ...
        assert open_path_calls == ["latest_download"]
        assert run["status"] == "completed"
        assert "agent.desktop.intent_planned" in event_types
        assert "agent.tool.call" in event_types
        assert "agent.desktop.intent_completed" in event_types
...
def test_send_message_executes_latest_screenshot_open_path_without_model(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    open_path_calls: list[str] = []
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct latest-screenshot task should not call model")
        ),
    )
    ...
    try:
        ...
        assert result["ok"] is True
        assert result["status"] == "completed"
        assert result["agent_task"]["summary"] == "已打开文件：~/Desktop/Screenshot.png。"
        assert result["agent_task"]["tool_calls"][-1]["tool_name"] == "desktop.open_path"
        assert result["agent_task"]["tool_calls"][-1]["input_preview"] == {"path": "latest_screenshot"}
        assert task is not None
        assert task.status == TaskStatus.COMPLETED
        assert task.result == "已打开文件：~/Desktop/Screenshot.png。"
        assert assistant is not None
        assert assistant.status == MessageStatus.COMPLETED
        assert assistant.content == "已打开文件：~/Desktop/Screenshot.png。"
        assert open_path_calls == ["latest_screenshot"]
        assert run["status"] == "completed"
        assert "agent.desktop.intent_planned" in event_types
        assert "agent.tool.call" in event_types
        assert "agent.desktop.intent_completed" in event_types
...
def test_send_message_executes_finder_selection_open_path_without_model(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    open_path_calls: list[str] = []
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct Finder-selection task should not call model")
        ),
    )
...
def test_send_message_executes_reveal_path_without_model(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    reveal_calls: list[str] = []
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct reveal path task should not call model")
        ),
    )
    ...
    try:
        ...
        assert result["ok"] is True
        assert result["status"] == "completed"
        assert result["agent_task"]["summary"] == "已在 Finder 中显示：~/Downloads/report.pdf。"
        assert result["agent_task"]["tool_calls"][-1]["tool_name"] == "desktop.reveal_path"
        assert result["agent_task"]["tool_calls"][-1]["status"] == "completed"
        assert task is not None
        assert task.status == TaskStatus.COMPLETED
        assert task.result == "已在 Finder 中显示：~/Downloads/report.pdf。"
        assert assistant is not None
        assert assistant.status == MessageStatus.COMPLETED
        assert assistant.content == "已在 Finder 中显示：~/Downloads/report.pdf。"
        ...
        assert result["agent_task"]["tool_calls"][-1]["tool_name"] == "desktop.reveal_path"
        assert result["agent_task"]["tool_calls"][-1]["input_preview"] == {"path": "~/Downloads"}
        assert task is not None
        assert task.status == TaskStatus.COMPLETED
        assert task.result == "已在 Finder 中显示：~/Downloads。"
        assert assistant is not None
        assert assistant.status == MessageStatus.COMPLETED
        assert assistant.content == "已在 Finder 中显示：~/Downloads。"
        assert reveal_calls[-1] == "~/Downloads"
        assert run["status"] == "completed"
        assert "agent.desktop.intent_planned" in event_types
        assert "agent.tool.call" in event_types
        assert "agent.desktop.intent_completed" in event_types
        ...
        assert result["ok"] is True
        assert result["status"] == "completed"
        assert result["agent_task"]["summary"] == "已在 Finder 中显示：.。"
        assert result["agent_task"]["tool_calls"][-1]["tool_name"] == "desktop.reveal_path"
        assert result["agent_task"]["tool_calls"][-1]["input_preview"] == {"path": "."}
        assert task is not None
        assert task.status == TaskStatus.COMPLETED
        assert task.result == "已在 Finder 中显示：.。"
        assert assistant is not None
        assert assistant.status == MessageStatus.COMPLETED
        ...
        assert result["ok"] is True
        assert result["status"] == "completed"
        assert result["agent_task"]["summary"] == "已在 Finder 中显示：finder_selection。"
        assert result["agent_task"]["tool_calls"][-1]["tool_name"] == "desktop.reveal_path"
        assert result["agent_task"]["tool_calls"][-1]["input_preview"] == {"path": "finder_selection"}
        assert task is not None
        assert task.status == TaskStatus.COMPLETED
        assert task.result == "已在 Finder 中显示：finder_selection。"
        assert assistant is not None
        assert assistant.status == MessageStatus.COMPLETED
        assert assistant.content == "已在 Finder 中显示：finder_selection。"
...
def test_send_message_executes_direct_clipboard_write_task(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct clipboard task should not call model")
        ),
    )
...
def test_send_message_executes_structured_open_path_recovery_action_without_model(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("structured open-path recovery should not call model")
        ),
    )
    ...
    try:
        result = api.send_message(
            ...
            metadata={
                ...
                "recovery_risk_level": "low",
                "source_task_id": "task-source-file",
                "source_task_title": "打开下载文件夹",
            },
        )
        task = runtime.state.get_task(result["task_id"])
        run = service.get_run(result["run_id"])
        ...
        assert assistant is not None
        assert assistant.status == MessageStatus.COMPLETED
        assert assistant.content == "已打开文件夹：~/Downloads。"
        assert open_path_calls == ["~/Downloads"]
        assert run["status"] == "completed"
        assert run["pending_approval"] == {}
        assert user_metadata["desktop_permission_recovery"] is True
...
def test_send_message_executes_structured_browser_open_recovery_action_without_model(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("structured browser recovery should not call model")
        ),
    )
    ...
    try:
        result = api.send_message(
            ...
            metadata={
                ...
                "recovery_risk_level": "low",
                "source_task_id": "task-source-browser",
                "source_task_title": "打开 GitHub",
            },
        )
        task = runtime.state.get_task(result["task_id"])
        run = service.get_run(result["run_id"])
        ...
        assert result["agent_task"]["needs_user_action"] is False
        assert result["agent_task"]["pending_approvals"] == []
        assert result["agent_task"]["summary"] == "已打开网页：https://github.com。"
        assert result["agent_task"]["tool_calls"][-1]["tool_name"] == "browser.open_url"
        assert result["agent_task"]["tool_calls"][-1]["input_preview"] == {"url": "https://github.com"}
        assert task is not None
        assert task.status == TaskStatus.COMPLETED
        assert task.result == "已打开网页：https://github.com。"
        assert assistant is not None
        assert assistant.status == MessageStatus.COMPLETED
        assert assistant.content == "已打开网页：https://github.com。"
...
def test_send_message_executes_structured_safe_foreground_recovery_actions_without_model(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("structured safe foreground recovery should not call model")
        ),
    )
...
def test_send_message_prepares_finder_find_open_first_then_waits_for_click_approval(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("finder find-open-first task should not call model")
        ),
    )
    monkeypatch.setattr(
        "apps.shell.chat_api.desktop_permission_missing_by_capability",
...
def test_send_message_executes_direct_spotlight_search_sequence_without_model(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct spotlight search should not call model")
        ),
    )
...
def test_send_message_routes_return_submit_to_approval_without_model(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    focus_calls: list[str] = []
...
def test_send_message_routes_app_scoped_close_window_to_approval_without_model(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("app scoped close-window approval task should not call model")
        ),
    )
    monkeypatch.setattr(
...
def test_send_message_executes_direct_windows_list_task(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    windows_calls: list[str] = []
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct windows list task should not call model")
        ),
    )
    ...
    try:
        ...
        assert result["ok"] is True
        assert result["status"] == "completed"
        assert result["agent_task"]["summary"] == "当前窗口：Google Chrome: ChatGPT; Finder: Downloads。"
        assert result["agent_task"]["tool_calls"][-1]["tool_name"] == "desktop.windows"
        assert result["agent_task"]["tool_calls"][-1]["input_preview"] == {}
        assert result["agent_task"]["tool_calls"][-1]["status"] == "completed"
        assert task is not None
        assert task.status == TaskStatus.COMPLETED
        assert task.result == "当前窗口：Google Chrome: ChatGPT; Finder: Downloads。"
        assert assistant is not None
        assert assistant.status == MessageStatus.COMPLETED
        assert assistant.content == "当前窗口：Google Chrome: ChatGPT; Finder: Downloads。"
        assert windows_calls == [""]
        assert run["status"] == "completed"
        assert "agent.desktop.intent_planned" in event_types
        assert "agent.tool.call" in event_types
...
def test_send_message_focuses_app_prefix_then_reads_ui_elements_without_model(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("app scoped UI elements task should not call model")
        ),
    )
...
def test_send_message_executes_direct_minimize_current_window_task(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct minimize window task should not call model")
        ),
    )
    ...
    def fake_minimize_window() -> dict:
        ...
        return {
            ...
            "summary": "Minimized current window",
            "data": {},
        }
...
def test_send_message_prepares_comm_find_message_then_waits_for_send_approval(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("communication compose approval should not call model")
        ),
    )
...
def test_send_message_executes_direct_foreground_find_text_task(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    calls: list[tuple[str, str]] = []
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct foreground find task should not call model")
        ),
    )
...
def test_send_message_executes_direct_safe_arrow_key_task(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct safe foreground key task should not call model")
        ),
    )
...
def test_send_message_executes_next_input_focus_as_safe_tab_key(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    pressed: list[tuple[str, int]] = []
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct next input focus task should not call model")
        ),
    )
...
def test_send_message_executes_previous_input_focus_as_safe_shift_tab_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct previous input focus task should not call model")
        ),
    )
...
def test_send_message_executes_direct_safe_scroll_page_task(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("direct safe foreground scroll task should not call model")
        ),
    )
    ...
    try:
        ...
        assert result["agent_task"]["needs_user_action"] is False
        assert result["agent_task"]["pending_approvals"] == []
        assert result["agent_task"]["summary"] == "已向下滚动前台界面（1 页）。"
        assert result["agent_task"]["tool_calls"][-1]["tool_name"] == "desktop.safe_scroll"
        assert result["agent_task"]["tool_calls"][-1]["input_preview"] == {
            "direction": "down",
            "pages": 1,
        }
        assert task is not None
        assert task.status == TaskStatus.COMPLETED
        assert task.result == "已向下滚动前台界面（1 页）。"
        assert assistant is not None
        assert assistant.status == MessageStatus.COMPLETED
        assert assistant.content == "已向下滚动前台界面（1 页）。"
        assert run["status"] == "completed"
        assert "agent.desktop.intent_completed" in event_types
        assert "model.request.started" not in event_types
        assert scrolled == [("down", 1)]
    finally:
        service.close()
        store.close()


def test_send_message_executes_app_prefix_safe_scroll_without_model(tmp_path, monkeypatch):
    api, runtime, store = _make_api(tmp_path)
    service = _make_agent_runtime_service(tmp_path)
    runtime.agent_runtime_service = service
    calls: list[tuple[str, str] | tuple[str, str, int]] = []
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("app scoped safe scroll task should not call model")
        ),
    )
    ...
    try:
        ...
        assert result["ok"] is True
        assert result["status"] == "completed"
        assert calls == [("focus", "Google Chrome"), ("scroll", "down", 1)]
        assert result["agent_task"]["status"] == "completed"
        assert result["agent_task"]["needs_user_action"] is False
        assert result["agent_task"]["pending_approvals"] == []
        assert result["agent_task"]["summary"] == "已切到 Google Chrome 并向下滚动前台界面（1 页）。"
        assert result["agent_task"]["tool_calls"][-1]["tool_name"] == "app.focus_and_safe_scroll"
        assert result["agent_task"]["tool_calls"][-1]["input_preview"] == {
            "app_name": "Google Chrome",
            "direction": "down",
            "pages": 1,
        ...
        assert task is not None
        assert task.status == TaskStatus.COMPLETED
        assert task.result == "已切到 Google Chrome 并向下滚动前台界面（1 页）。"
        assert assistant is not None
        assert assistant.status == MessageStatus.COMPLETED
        assert assistant.content == "已切到 Google Chrome 并向下滚动前台界面（1 页）。"
        assert run["status"] == "completed"
        assert "agent.desktop.intent_completed" in event_types
        assert "model.request.started" not in event_types
        assert "model.requested" not in event_types
...
def test_send_message_safe_type_text_defaults_to_provider_protection(
    ...
    monkeypatch.setattr(
        "apps.shell.agent_runtime.openai_compatible_chat_message",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("provider-protected direct input should not call model first")
        ),
    )
    monkeypatch.setattr(
        "apps.shell.agent.tools.desktop.desktop_safe_type_text",
...
def test_send_message_prepares_app_safe_type_text_then_waits_for_send_approval(
    tmp_path,
    monkeypatch,
):
    api, runtime, store = _make_api(tmp_path)
