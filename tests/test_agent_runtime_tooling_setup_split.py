...
def test_build_runtime_tooling_wires_executor_runner_and_shared_loop_projection() -> None:
    ...
    def allows_tool(_tool_name, _allowed_tools):
        return True
...
def test_build_runtime_tooling_stack_wires_policy_budget_and_custom_loop() -> None:
    ...
    assert (
        stack.tooling.tool_call_executor._validate_tool_payload
        is RuntimeToolOperations.validate_tool_payload
    )
...
def test_native_runtime_installs_tooling_bundle_under_legacy_attribute_names(tmp_path) -> None:
    ...
    try:
        ...
        assert (
            service.tool_request_runner._pending_approval_builder is service.tool_pending_approvals
        )
        ...
        assert (
            service.tool_call_executor._validate_tool_payload
            is RuntimeToolOperations.validate_tool_payload
        )
        assert (
            getattr(service.tool_call_executor._limit_tool_result, "__self__", None) is not service
        )

        service.runtime_limits = RunBudgetLimits(max_tool_output_chars=5)
        limited = service.tool_call_executor._limit_tool_result(
            {"ok": True, "content": "abcdefghi"}
        )
