...
- ✅ Agent 审批体验改为独立工具请求卡片：关联任务、工具名、请求参数分层展示，`terminal.run` 以 bash 代码块显示；Workflow 人工审批会展示审批节点、审批说明和当前上下文；Run Detail 的 Workflow Step 审批摘要不再被前端二次截断，长内容交给折叠控件展示；输入框上方会浮出待审批提醒并支持多审批切换，提醒条会优先显示 Workflow 审批节点/说明，并同时提供“定位消息”和“运行详情”，方便用户从输入区回到原气泡或打开 Run Detail 确认上下文；批准后会立即释放按钮 busy 并后台轮询 Agent 继续执行进度，后端会先把子 Agent Run 和等待它的父 Workflow 标记为 running、清理旧审批并写入 resumed timeline，Chat 进度卡和 Run Detail timeline 也会把 resumed 事件显示成“已继续执行”而不是内部事件名，再继续跑工具/模型，完成后才触发主模型汇总。
- ✅ 2026-06-06 追加：Agent 工具审批通过/拒绝都会写入明确 timeline 事件，包含工具名、参数预览和状态；Run Detail 会显示“审批已通过 · terminal.run”这类标题，拒绝时也保留拒绝原因和原始请求预览，复盘时不再需要从后续 tool call 倒推用户批准了什么。
- ✅ 2026-06-06 追加：Workflow 人工审批通过/拒绝 timeline 也会保留同一份审批预览，包括审批节点、审批说明和当时上下文；Run Detail 标题会显示“审批已通过 · 人工确认”或“审批已拒绝 · 原因”，用户复盘 Workflow 时能直接看到当时批准/拒绝的具体内容。
- ✅ 2026-06-06 追加：Run Detail timeline 的审批完成事件会使用完成态视觉，而不是继续沿用待审批色；待审批、已通过、已拒绝三种状态在复盘时更容易区分。
...
- ✅ 2026-06-06 追加：Chat 底部状态行现在也会优先显示输入框上方当前待审批项，包括从 activity 或父 Workflow 派生出来的子 Agent 审批；多审批时显示序号，文案带请求者、工具名和命令/审批节点摘要，避免浮条已经出现但状态仍停在泛泛“处理中”。
...
- ✅ Workflow 入口边界按最新目标调整：Chat `@` 候选会显示主模型、Agent 和 Workflow；用户手动 `@Workflow` 会直接创建 Workflow Run，并把 Workflow 总结与子 Agent 消息写回当前对话；主会话显式触发会切到 Workflow 会话，群聊里显式触发会保持群聊上下文且不会把 Workflow 加成群成员，同时会继续扫描前一个已完成的直接 `@Agent` 结果并交给主模型整理；主模型在群组派发协议里误派发 Workflow 仍会提示去 Workflow Studio / Runs，而不会把 Workflow 当成群组 Agent 自动执行；Workflow 的设计、保存、保存并运行和 Runs 手动运行仍保持可用，且保存更新后立即运行会使用最新画布节点/连线，不会跑到旧版本；Bridge route 层也覆盖 create/update/run 这条用户实际点击路径。
- ✅ 2026-06-06 追加：Chat 手动 `@Workflow` 触发已改为异步 Workflow Run；发送后会立即出现 Workflow processing 气泡和运行进度摘要，不再阻塞等待整个流程完成。后台完成、失败、取消或进入审批后会通过现有 Run 同步逻辑更新同一条 Workflow 气泡，群聊里显式触发 Workflow 也会保留群组上下文并继续补建前一个直接 `@Agent` 的主模型汇总任务。
...
- ✅ 2026-06-05 追加，2026-06-06 更新：Chat 中手动 `@Workflow` 已从“引导到 Studio / Runs”升级为直接运行入口；`@` 菜单会展示 Workflow，运行后仍可从气泡进入 Run Detail 复盘步骤与产物。群组主模型派发协议仍只执行群内 Agent，误派 Workflow 时继续给出 Workflow Studio / Runs 引导，避免自动编排和显式流程触发混在一起。
