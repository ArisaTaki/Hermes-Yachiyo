...
- ✅ apps/shell/chat_bridge.py（新建）— 统一聊天摘要桥接层
  ...
  - `get_session_status()` — 会话状态（无消息/处理中/就绪）
...
- ✅ apps/shell/modes/live2d.py — 角色舞台增加聊天能力
  ...
  - JS 增加 `sendMsg()` / `refreshMessages()` / 1200ms 轮询
  - 角色图标交互：处理中 ⚡ / 空闲 🎤
  - 保留全部原有非聊天方法
- ✅ tests/test_chat_bridge.py（新建）— 15 个测试用例
...
| Bubble | 轻量浮窗入口 | 摘要 + 快捷发送（ChatBridge） | 1200ms |
| Live2D | 角色舞台入口 | 摘要 + 快捷发送（ChatBridge） | 1200ms |
