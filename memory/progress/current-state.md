...
- ✅ 2026-06-05 追加：群组派发解析继续兼容工具调用风格 envelope，例如 `{"tool":"dispatch_group_agent","input":{...}}`、`arguments/parameters/params/payload/request` 包一层，或 arguments 是 JSON 字符串；这类真实模型常见格式会被拆出 Agent 和目标并隐藏机器块，不再因为多包一层就跳过派发。
- ✅ Agent 审批体验改为独立工具请求卡片：关联任务、工具名、请求参数分层展示，`terminal.run` 以 bash 代码块显示；Workflow 人工审批会展示审批节点、审批说明和当前上下文；输入框上方会浮出待审批提醒并支持多审批切换，提醒条会优先显示 Workflow 审批节点/说明，并同时提供“定位消息”和“运行详情”，方便用户从输入区回到原气泡或打开 Run Detail 确认上下文；批准后会立即释放按钮 busy 并后台轮询 Agent 继续执行进度，完成后才触发主模型汇总。
...
- ✅ 2026-06-05 追加：Workflow Studio 的 Workflow Run 区域会按当前画布显示运行顺序预览，列出将要执行的 Agent / Approval / Artifact 步骤、Step Task、审批说明和预计 artifact 路径；预览和保存请求复用同一套节点/连线组装逻辑，减少“画布看到的”和“实际保存运行的”不一致。
