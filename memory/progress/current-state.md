...
- ✅ 2026-06-06 追加：父 Workflow 等待子 Agent 工具审批时，聊天摘要会直接显示等待对象、Workflow 节点和审批工具，并把 `workflow_waiting_child_run_id` / `workflow_waiting_node` / `workflow_waiting_tool` 写入 metadata；Chat 输入区待审批提醒会在缺少子 Agent 审批气泡时，用这组 metadata 派生一个指向子 Run 的提醒项，真实子 Agent 审批消息或 activity 已存在时不会重复；`approval_count` 也会按实际待处理 Run ID 去重计数，即使只剩父 Workflow 消息也能让会话列表和 session info 显示待审批；子 Agent 审批通过后如果父 Workflow 恢复失败，失败 timeline 也会保留关联 `child_run_id`、子 Run 状态和节点信息，Run Detail 能定位到具体失败节点。
