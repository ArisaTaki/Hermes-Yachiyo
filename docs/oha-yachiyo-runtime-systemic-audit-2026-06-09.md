...
- 本轮还把 release workflow 的最终 RC gate 从只做 `--check-dmg-mount` 提升为默认加 `--run-dmg-app-smoke`：CI 上传 artifacts 前会只读挂载 DMG、启动 DMG 内真实 `.app`、等待 packaged Bridge `/status`，通过后同一份 `release/rc-verification.json` 会自动把 `packaged_bridge_isolation` 标为 `passed`。这样 release workflow 归档草稿默认不再要求签核人手工证明 packaged bridge isolation；Gatekeeper、Screen Recording、OS file picker 和跨页面 packaged UI 抽样仍保持对应人工/授权路径。
...
5. 做最终发布验收切片：
   ...
   - 当前实际产出的 `.app` / DMG 已在同一次批量 RC gate 中通过 `--check-dmg-mount`、`--run-dmg-app-smoke` 和 `--run-ui-smoke`；最终发布前剩余人工确认首次启动文案、Gatekeeper 打开路径、原生图片 file picker、packaged UI 抽样，以及在具备真实 provider credentials 时运行带 `--run-provider-smoke` 的 RC gate。屏幕录制权限可优先用 `--run-dmg-screen-smoke` 自动收证，失败时再回退到人工记录 System Settings 状态和截图/proactive probe 结果。
