...
- 本轮还把 `--manual-checks-json` 扩展为可重复传入：最终签核可先传自动 RC report，再传只包含人工补充项的小 JSON；脚本按命令行顺序合并，后传入文件可覆盖前一份里的同一 check，同一文件内部重复 id 仍会报错。这样 `packaged_bridge_isolation`、UI smoke supporting notes、provider not_applicable evidence 和人工 Gatekeeper / screen / file picker / UI sampling evidence 不需要复制到同一个大文件里。
...
- 本轮新增 `--run-dmg-chat-native-file-smoke`：它会挂载 DMG、启动真实 packaged Electron app，并在显式 `OHA_YACHIYO_DESKTOP_SMOKE_MODE=1` 下让主进程 `chooseChatImages` IPC 读取脚本生成的本地图片路径；随后通过 Chromium DevTools 点击 Chat 附件按钮，验证 preview、send payload、message attachment、image viewer open/close 和 Run Detail replay handoff。通过时 RC report 会写入 `dmg_chat_native_file_smoke.uploads` 并把 `chat_native_file_upload` 自动标为 `passed`；正常用户运行不设置 smoke env，仍使用系统原生 file picker。该 gate 已有 verifier 单测和 release artifact guard，实际 DMG 通过证据需等下一次重新打包后的 RC artifact 运行。
...
1. 做最终发布签核批处理：
   - 以 `tmp/rc-verification-packaged-ui.json` 作为当前自动 evidence source，先运行 `python scripts/verify_release_candidate.py --manual-checks-json tmp/rc-verification-packaged-ui.json --mark-provider-smoke-not-applicable-if-missing --write-manual-checks-draft tmp/rc-signoff-after-packaged-ui.json` 生成可编辑签核草稿；release workflow 也会在 `release/rc-verification.json` 生成后归档 `release/manual-rc-checks.draft.json` 和可读的 `release/manual-rc-checks.md`。无 provider credentials 的发布签核人当前只需补齐 `gatekeeper_first_launch`、`screen_recording_permission` 和 `chat_native_file_upload` 三项；屏幕录制可在授权后用 `python scripts/verify_release_candidate.py --require-artifacts --run-dmg-screen-smoke --report-json tmp/rc-verification-screen.json` 自动填 `screen_recording_permission` evidence；重新打包后可用 `python scripts/verify_release_candidate.py --require-artifacts --run-dmg-chat-native-file-smoke --report-json tmp/rc-verification-chat-native-file.json` 自动填 `chat_native_file_upload` evidence。有真实 provider credentials 时仍应运行带 `--run-provider-smoke` 的 RC gate，而不是把 provider 标为 `not_applicable`。
