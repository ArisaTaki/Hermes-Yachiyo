...
- 桌面 `.app` 已实际启动并验证 bridge；主要 UI 页面已有静态入口 guard、浏览器级 route smoke、source Browser 按钮级交互 smoke、DMG 内 packaged app startup smoke 和十九个本地 Electron UI smoke。新增 `--run-dmg-ui-sampling-smoke` 后，release candidate 包的跨页面 packaged renderer 抽样可走自动 evidence；剩余 UI 人工重点收敛到真实 OS file picker 弹窗、Gatekeeper/Finder 首启路径和 Screen Recording 授权。

## 下一步建议

1. 做最终发布签核批处理：
   - 以 `tmp/final-rc.json` 作为自动 evidence source，先运行 `python scripts/verify_release_candidate.py --manual-checks-json tmp/final-rc.json --write-manual-checks-draft tmp/final-rc-signoff.json` 生成可编辑签核草稿；release workflow 也会在 `release/rc-verification.json` 生成后归档 `release/manual-rc-checks.draft.json` 和可读的 `release/manual-rc-checks.md`。发布签核人只需补齐 `gatekeeper_first_launch` 和 `chat_native_file_upload` 两个人工项；`packaged_ui_sampling` 可用 `python scripts/verify_release_candidate.py --require-artifacts --run-dmg-ui-sampling-smoke --report-json tmp/rc-verification-packaged-ui.json` 自动填 evidence，屏幕录制可在授权后用 `python scripts/verify_release_candidate.py --require-artifacts --run-dmg-screen-smoke --report-json tmp/rc-verification-screen.json` 自动填 `screen_recording_permission` evidence。如果最终发布环境明确没有真实 provider credentials，可在 draft / Markdown checklist 生成或最终 `--require-manual-checks-complete` gate 中加 `--mark-provider-smoke-not-applicable-if-missing`，把 `real_provider_smoke` 标为 `not_applicable` 并附 credentials-unavailable evidence。
