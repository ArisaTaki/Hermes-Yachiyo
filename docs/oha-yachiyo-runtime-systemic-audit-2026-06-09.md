...
- message-level SSE tool-call、multiline `data:` SSE tool-call 与 Responses-style NativeRunEngine 主循环回归已纳入 macOS release workflow smoke，release verifier 会阻断 workflow 丢失这些测试，避免发布门禁只覆盖普通 delta 风格 Chat Completions SSE 而漏掉 message-level、多行 SSE transport 或 Responses 风格 tool-call stream。
