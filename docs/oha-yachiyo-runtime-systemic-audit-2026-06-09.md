...
- release workflow smoke 现在也强制包含 sensitive HTTP `Idempotency-Key` 回归，确保 Chat route 不创建消息/Task，Agent/Workflow route 的错误响应也不会回显 header secret。
