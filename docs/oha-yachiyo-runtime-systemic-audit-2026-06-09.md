...
- 可重复本地 UI smoke `node scripts/smoke_agent_run_detail_ui.mjs` 会启动 mock Bridge、Vite dev server 和 Electron BrowserWindow，加载 `#/agents/{run_id}`，seed 一个 completed Native Agent Run、TaskRunLink 投影、RunGroup 和 201 条 `/runs/{run_id}/events` replay facts；首屏断言 Run Detail article、Task、Result、前 200 条 `agent-run-detail-execution-event` 的 `data-run-event` / sequence / run-id 属性和 `agent-run-detail-load-more-events`，点击加载更多后再断言第 201 条 completed replay 合并进同一个 Run 且 load-more 消失。source-level pytest guard 锁定该 smoke 必须走 replay route、稳定 DOM 属性和分页 UI。
