...
- 新增可重复本地 UI smoke：`node scripts/smoke_chat_image_attachment_ui.mjs` 会启动 mock Bridge、Vite dev server 和 Electron BrowserWindow，加载 Chat 页面，通过 dev-only image event 注入图片，断言 composer preview、`/ui/chat/messages` 发送 payload、message attachment item 和 image viewer modal；本轮已实际运行通过。
