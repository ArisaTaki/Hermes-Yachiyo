...
- 主聊天 provider streaming completed RunEvent 现在不只保留合并后的可见 output，也会在 provider 提供时把 `finish_reason` 以及 `prompt_tokens` / `completion_tokens` / `total_tokens` / `input_tokens` / `output_tokens` / `cached_tokens` 等 numeric usage 写入单条 `model.output.completed` payload；dict/SSE chunk 与 OpenAI SDK object-style chunk 已有回归，仍不写 token/delta 级 durable event。
...
- 可重复本地 UI smoke `node scripts/smoke_chat_image_attachment_ui.mjs` 会启动 mock Bridge、Vite dev server 和 Electron BrowserWindow，加载 Chat 页面，先在无桌面 picker API 时点击 header / composer 两个图片上传按钮并拦截 hidden `chat-image-file-input.click()`，确认 Web fallback 仍指向同一个文件输入；随后注入模拟的桌面 `chooseChatImages` API，点击 composer 附件按钮并确认不会触发 hidden input click，而是从桌面 picker API 返回的 data URL 生成 composer preview；再在页面上下文构造真实 `File` 并通过 hidden `chat-image-file-input` 的 `DataTransfer.files` 派发 `change`，断言 composer preview，点击 `chat-composer-attachment-remove` 后确认预览消失且发送按钮重新 disabled；最后通过 Chromium DevTools Protocol `DOM.setFileInputFiles` 给同一个 hidden file input 设置磁盘上的 `smoke-image-cdp.svg`，断言 composer preview、`/ui/chat/messages` 发送 payload、message attachment item，并真实点击消息附件打开 image viewer modal 后再点击 `chat-image-viewer-close`，确认 backdrop / modal / stage 均关闭。该 smoke 不依赖 dev-only image event，覆盖 `button → chooseChatImages → preview → remove`、`button → input.click()` fallback、`input.files → onChange → addImageFiles() → FileReader → preview → remove → DOM.setFileInputFiles → send → message attachment click → image viewer open/close` 路径，并由 source-level pytest guard 锁定不能退回 `oha-chat-e2e-add-image` 注入。
...
- Chat 图片附件的大图预览 modal / stage / close action 已补稳定 `data-testid`，本地 Electron smoke 现在也断言“点击消息附件 → 预览打开 → 关闭预览”的交互闭环；同一 smoke 还锁定 composer preview 的 id/mime/size/width/height、提交给 `/ui/chat/messages` 的 client attachment id/mime/size/dimensions/data URL，以及消息附件 DOM 的 id/kind/mime/name/size 元数据，避免图片 UI 上传路径丢失附件 handoff 信息。release verifier 现在还会检查 `scripts/smoke_chat_image_attachment_ui.mjs` 必须保留桌面 `chooseChatImages` API 路径、hidden input fallback、Chromium CDP `DOM.setFileInputFiles`、真实文件路径和四图 file input 覆盖，防止该 smoke 退化为只 dispatch change 的弱模拟。
...
- 新增本地 RC 验收入口 `python scripts/verify_release_candidate.py --require-artifacts`：先运行 source-level release guard，再对 `dist/backend`、`dist/electron` 和 `release` 执行 binary/package verifier；需要确认 DMG 内真实 `.app` 时可加 `--check-dmg-mount`，需要把 Electron UI smoke 纳入本地 RC gate 时可加 `--run-ui-smoke`，该路径会复用 `scripts/run_electron_ui_smokes.py` 的动态 discovery / report runner。该脚本会列出 Gatekeeper 首启、Bridge 启动隔离、屏幕录制权限和真实 provider smoke 等人工复验项，并可写出 `release/rc-verification.json` 作为可归档验收报告；release workflow smoke 也会运行其单测，release workflow 会在生成 release metadata 后、上传 DMG 前运行带 `--check-dmg-mount` 的 RC gate，若真实 provider smoke secrets 完整则向同一个 RC gate 传入 `--run-provider-smoke`，并把包含 `provider_smoke` 结果的 report 随 release artifacts 上传；release verifier 会阻断 RC gate 测试、workflow 步骤、provider smoke 参数或 report 输出漏接入发布门禁。
- 2026-06-12 起收尾切换为验收驱动：先跑 release / RC gate 找真实阻断项，再修阻断项，不再继续做低收益 runtime 微边界拆分。本轮 `python scripts/verify_release_artifacts.py` 已通过；`python scripts/verify_release_candidate.py --report-json tmp/release-candidate-dry-run.json` 的 source release guards 通过，但自动拾取到本机旧 `dist/electron` 并因 `app.asar` 缺最新 packaged selector 失败，证明旧 `.app` 不能计作 RC 产物。新增 `python scripts/verify_release_candidate.py --source-only --report-json tmp/source-only-rc.json` 用于未重新打包时只验证源码级 release guard；重打包后的最终 RC 产物已进入 `--require-artifacts` / `--check-dmg-mount` 验收路径。
...
- release artifact verifier 单测现在会通过 `scripts/run_electron_ui_smokes.py` 动态发现十九个 Electron UI smoke 脚本，并通过动态 packaged selector gate 验证这些脚本显式使用的 `data-testid` 都会进入 `.app` 产物扫描范围，避免后续 smoke 继续新增成熟 UI 入口时漏同步 packaged app selector gate。
- release artifact verifier 现在会动态发现 `scripts/smoke_*_ui.mjs`，要求 release workflow 在 packaging 前运行动态 Electron UI smoke runner；单测会模拟新增成熟 UI smoke 不需要改 workflow 的场景，避免后续新增 Browser/Desktop 回归脚本时继续维护手工 workflow 列表。
...
- release workflow smoke 现在会在 packaging 前通过 `python scripts/run_electron_ui_smokes.py --report-json release/electron-ui-smoke.json` 动态运行 Chat 图片附件、Chat completed message Run Detail handoff、Chat Agent progress Run Detail handoff、Chat 取消、Chat 审批、Chat delegated summary、群聊 summary、Activity feed/detail、Diagnostics local screenshot、Live2D settings、launcher session summary、Proactive TTS、Agent Studio agents 定义 CRUD、Agent Studio Skill Library、Agent Studio Skill mounting、Agent Studio Skill folders、Agent Run Detail replay / Workflow child approval、Workflow save-and-run 和 Workflow management/delete 十九个 Electron UI smoke，并把结果作为 `release/electron-ui-smoke.json` 随 release artifacts 上传；本地 RC `--run-ui-smoke` 也复用同一份 runner，release verifier 会阻断 workflow 丢失 runner、RC verifier 回退到私有 UI smoke 执行逻辑、runner 不再动态发现 `scripts/smoke_*_ui.mjs` 或不再输出结构化 report。
- RC verifier 的 opt-in 真实 provider 文本流 smoke 现在要求 `finish_reason=stop`，tool-call smoke 要求 `finish_reason=tool_calls` 且要求 synthetic `workspace_read` tool-result 后第二轮 stream 输出内容与 `finish_reason=stop`；release verifier 会阻断 `verify_release_candidate.py` 丢失任一断言，避免真实 provider 未完成标准结束协议或不接受 tool-result history 时误过发布门禁。release verifier 也会要求 workflow 在 `OHA_YACHIYO_SMOKE_BASE_URL`、`OHA_YACHIYO_SMOKE_MODEL` 或 `OHA_YACHIYO_SMOKE_API_KEY` 任一缺失时显式 skip 并输出 opt-in secret 未完整配置提示，且在 secrets 完整时把 `--run-provider-smoke` 拼进最终 RC gate，避免无凭据环境误跑真实 provider smoke 或有凭据环境只跑旁路 smoke 而未归档。
...
- NativeRunEngine 现在会把 OpenAI-compatible stream chunk 上的 `stop_reason` 归一化进既有 `finish_reason` metadata，主聊天和 Agent Run 都会在 `model.output.completed` RunEvent 中保留该终止原因；Agent Run final output 会先写 `model.output.completed`，再写 `agent.run.completed`。新增的主聊天与 Agent Run stop_reason 回归已进入 release workflow smoke 和 release artifact verifier，避免真实 gateway 用 `stop_reason` 而非 `finish_reason` 时 RunEvent replay 丢失 provider 结束协议。
- NativeRunEngine 现在也和 opt-in streaming smoke helper 对齐，支持 OpenAI-compatible gateway 把 stream chunk 展平成顶层 `delta.tool_calls` / `message.tool_calls` 的形态；主聊天和 Agent Run 都会把这些 frame 归并成同一个 `workspace.read` tool call、执行 tool-result follow-up，并只落完成态 RunEvent。新增回归已进入 release workflow smoke 和 release artifact verifier，避免真实 gateway 预处理掉 `choices[]` 包装时 smoke helper 通过但产品 runtime 漏掉工具调用。
...
- release candidate verifier 已新增 `--check-dmg-mount`：本地会只读挂载发现到的 DMG，并对 DMG 内真实 `Oha-Yachiyo.app/Contents/Resources` 和所属 `.app` bundle 再跑 packaged app scan；release workflow 上传 DMG 前会运行 `python scripts/verify_release_candidate.py --require-artifacts --check-dmg-mount --run-dmg-app-smoke --report-json release/rc-verification.json`，同时验证 DMG 内 `.app` 可启动并暴露 packaged Bridge。本机沙箱内直接 `hdiutil attach` 会失败，提升权限后 `python scripts/verify_release_candidate.py --require-artifacts --check-dmg-mount --report-json tmp/rc-verification-local-dmg.json` 已通过，确认当前 DMG 内 `.app` 与 unpacked `.app` 一致通过发布门禁。
...
- release candidate verifier 已新增 opt-in `--run-dmg-screen-smoke`：本地会只读挂载 DMG、启动 DMG 内 `Oha-Yachiyo.app`、等待 packaged backend `/status`，再请求 `/screen/current` 验证 Screen Recording 授权和本地截图路径。通过时 report 只记录截图 width/height/format/captured_at 元数据，不归档 `image_base64`；同一轮 gate 会把 `packaged_bridge_isolation` 和 `screen_recording_permission` 自动标为 `passed`，从最终签核人工项里移除屏幕录制手填。
- 本轮修正了 DMG packaged app smoke 的 cleanup 边界：`--run-dmg-app-smoke` 和 `--run-dmg-screen-smoke` 现在用独立 process group 启动 DMG 内 `.app`，cleanup 时终止整个进程组，避免 Electron helper / packaged backend 残留导致同一轮后续 screen probe 卡在旧实例或新 Bridge 端口连接失败。修正后运行 `python scripts/verify_release_candidate.py --require-artifacts --check-dmg-mount --run-dmg-app-smoke --run-dmg-screen-smoke --report-json tmp/final-rc-dmg-screen-after-fix.json`：source release guards、built artifact guards、DMG mount guards 和 DMG app startup smoke 均通过，`packaged_bridge_isolation` 自动标为 `passed`；screen probe 已能连到 packaged `/screen/current`，当前失败原因收敛为真实 `screen_capture_permission_denied` 授权缺口，而不是 RC gate 启动/清理不可靠。
- release candidate verifier 已新增 opt-in `--run-dmg-ui-sampling-smoke`：本地会只读挂载 DMG、启动真实 packaged `.app`、等待 packaged `/status`，再用 Chromium DevTools 抽样 Chat、Agent Studio、Workflow、Activity、Diagnostics、Proactive TTS 和 Live2D settings 的稳定 selector。通过时同一轮 report 会把 `packaged_bridge_isolation` 和 `packaged_ui_sampling` 自动标为 `passed`；Chat native-file upload 由单独的 `--run-dmg-chat-native-file-smoke` 自动收证。
...
- release candidate report 现在还会写入结构化 `manual_release_candidate_check_statuses` 和 `manual_release_candidate_check_summary`，把 `gatekeeper_first_launch`、`packaged_bridge_isolation`、`screen_recording_permission`、`chat_native_file_upload`、`packaged_ui_sampling` 和 `real_provider_smoke` 六个最终验收项标成稳定 id，并记录每项 release signoff 前需要附上的 evidence 和下一步动作；旧的 `manual_release_candidate_checks` 字符串列表保留兼容。summary 会输出 `remaining_count`、`remaining_check_ids`、`remaining_next_actions`、`failed_check_ids` 和 `automated_evidence_check_ids`，后续判断“还差多少”和下一步该跑哪个 gate / 做哪项人工复验不再需要人工数列表。默认状态仍是 `manual_required`，但通过同一次 RC gate 的 `--run-dmg-app-smoke` 会把 `packaged_bridge_isolation` 自动标为 `passed` 并写入 `evidence_source=automated_rc_gate`，通过 `--run-dmg-screen-smoke` 会把 `packaged_bridge_isolation` 和 `screen_recording_permission` 自动标为 `passed`，通过 `--run-dmg-ui-sampling-smoke` 会把 `packaged_bridge_isolation` 和 `packaged_ui_sampling` 自动标为 `passed`，通过 `--run-provider-smoke` 会把 `real_provider_smoke` 自动标为 `passed`；自动 evidence 只填充仍为 `manual_required` 的项，不覆盖签核人已写入的 `passed` / `failed` / `not_applicable`。`--write-manual-checks-template` 可生成带 `evidence_prompt` 的项目内 JSON 模板，release workflow 会把 `release/manual-rc-checks.template.json` 随 DMG/RC report 一起上传；`--manual-checks-json` 可从项目内 JSON 合并人工 evidence，支持顶层 list、`{ "checks": [...] }` 或上一轮 RC report 的 `manual_release_candidate_check_statuses`，让自动 evidence 能直接进入最终 signoff rerun 而不需要复制模板；支持状态为 `manual_required` / `passed` / `failed` / `not_applicable`，未知 id、重复 id、非法 status、缺 evidence 或显式 failed 都会让 RC gate 失败并写入 `manual_release_candidate_check_findings`；最终发布签核可加 `--require-manual-checks-complete`，任何在自动 evidence 和人工 evidence 合并后仍为 `manual_required` 的检查都会失败。release verifier 会阻断 RC verifier、release workflow 或 release packaging 文档丢失这些字段、关键 id、summary、next actions、自动 evidence 合并、模板输出、evidence 输入、上一轮 RC report 复用或最终签核强制入口。
- 本轮将 `--mark-provider-smoke-not-applicable-if-missing` 从“只生成 draft / Markdown checklist 时生效”提升为最终 RC gate 也可显式使用：如果 `real_provider_smoke` 仍是 `manual_required` 且当前环境缺任一 `OHA_YACHIYO_SMOKE_*` 变量，RC report 会自动写入 `not_applicable` 与缺失变量 evidence；如果真实 provider smoke 已通过、失败或签核人已手工标记，则不会覆盖。这样无 provider credentials 的最终签核可直接运行带该 flag 的 `--require-manual-checks-complete` gate，不再需要为了这一项手工编辑 JSON。
- 本轮还把 `--manual-checks-json` 扩展为可重复传入：最终签核可先传多份自动 RC report，再传只包含人工补充项的小 JSON；previous RC report 合并时，后一份 report 里的 `manual_required` 不再抹掉前一份已经通过、failed 或 not_applicable 的自动 evidence，普通 `{ "checks": [...] }` 人工 evidence 仍可按命令行顺序覆盖同一 check，同一文件内部重复 id 仍会报错。这样 `packaged_bridge_isolation`、UI smoke supporting notes、provider not_applicable evidence 和人工 Gatekeeper / screen / file picker / UI sampling evidence 不需要复制到同一个大文件里。
- 本轮修正活跃阶段记录 `docs/model-profile-runtime-notes.md` 的旧执行链描述：该文档不再说主对话仍由旧 CLI 适配器执行，也不再使用旧 provider 映射、旧 runtime 名称或旧 `~/.hermes/...` 缓存路径；当前口径改为 Profile 驱动 Native provider，主对话 / Agent / Workflow / 委派 / 会话总结统一走 `NativeRunEngine`。`tests/test_legacy_kernel_removal.py` 已把该文档纳入 active user-facing docs guard，防止后续重新写入旧执行内核身份。
- 本轮优化最终签核批处理输出：`--write-manual-checks-draft` 和 `--write-manual-checks-markdown` 现在生成文件后会直接打印 `manual release-candidate check progress`、remaining ids 和 next actions。后续用上一轮 RC report 或小 JSON 合并 evidence 时，可以快速回答“还差多少 / 下一步做哪项”，不需要为了看剩余项重跑完整 artifact、DMG 或 UI gate；该优化不把 `chat_native_file_upload` 或 `packaged_ui_sampling` 的辅助 smoke 自动算作通过。
- 本轮把 release workflow 的 provider smoke 签核材料也接到同一批处理路径：如果三项 `OHA_YACHIYO_SMOKE_*` secrets 不完整，workflow 仍跳过真实 provider smoke，但会把 `--mark-provider-smoke-not-applicable-if-missing` 传给 `release/rc-verification.json`、`release/manual-rc-checks.draft.json` 和 `release/manual-rc-checks.md` 生成命令，让归档 signoff 文件直接包含 `real_provider_smoke=not_applicable` 与缺失变量 evidence；secrets 完整时仍运行 `--run-provider-smoke`，不降低真实 provider gate。
- 本轮还把 release workflow 的最终 RC gate 从只做 `--check-dmg-mount` 提升为默认加 `--run-dmg-app-smoke`：CI 上传 artifacts 前会只读挂载 DMG、启动 DMG 内真实 `.app`、等待 packaged Bridge `/status`，通过后同一份 `release/rc-verification.json` 会自动把 `packaged_bridge_isolation` 标为 `passed`。这样 release workflow 归档草稿默认不再要求签核人手工证明 packaged bridge isolation；packaged UI 抽样可用本地 `--run-dmg-ui-sampling-smoke` 自动收证，Chat native-file upload 可用本地 `--run-dmg-chat-native-file-smoke` 自动收证，Gatekeeper 和 Screen Recording 授权仍保持对应人工/授权路径。
- 本轮把已归档的 `release/electron-ui-smoke.json` 接入 manual signoff draft：`--manual-checks-json` 现在可接受 `scripts/run_electron_ui_smokes.py` 的 standalone report 作为 supporting evidence source，只写入 `packaged_ui_sampling` 和 `chat_native_file_upload` 的 Notes，不改变这两项的 `manual_required` 状态。release workflow 生成 `release/manual-rc-checks.draft.json` 时会同时读取 `release/rc-verification.json` 和 `release/electron-ui-smoke.json`，避免为了把 UI smoke supporting notes 带进签核草稿而重复跑一遍完整 Electron UI smoke。
- 本轮已按批量 RC 路线合并运行 `python scripts/verify_release_candidate.py --require-artifacts --check-dmg-mount --run-dmg-app-smoke --run-ui-smoke --report-json tmp/final-rc.json`，source release guards、built artifact guards、DMG mount guards、DMG app startup smoke 和 19 个 Electron UI smoke 均通过；修正 DMG smoke cleanup 后又运行 DMG-only screen gate，确认 `manual_release_candidate_check_summary.remaining_count` 为 5 项，`automated_evidence_check_ids` 为 `packaged_bridge_isolation`，screen 剩余原因是系统 Screen Recording 未授权。当前本机三项 `OHA_YACHIYO_SMOKE_*` env 仍缺失，`provider_smoke` 保持 skipped；使用 `python scripts/verify_release_candidate.py --manual-checks-json tmp/final-rc-dmg-screen-after-fix.json --mark-provider-smoke-not-applicable-if-missing --write-manual-checks-draft tmp/final-rc-signoff-provider-na.json` 后，草稿中 `real_provider_smoke` 自动标为 `not_applicable`，剩余项降为 4 个：`gatekeeper_first_launch`、`screen_recording_permission`、`chat_native_file_upload` 和 `packaged_ui_sampling`。
- 本轮又用 `tmp/final-rc-signoff-no-provider.json` 运行最终签核预览：`python scripts/verify_release_candidate.py --require-artifacts --manual-checks-json tmp/final-rc-signoff-no-provider.json --require-manual-checks-complete --report-json tmp/final-rc-signed-preview.json`。source release guards 和 built artifact guards 均通过，`packaged_bridge_isolation` 已保留自动 evidence，`real_provider_smoke` 已以 `credentials_unavailable` 标为 `not_applicable`；失败原因只剩 `gatekeeper_first_launch`、`screen_recording_permission`、`chat_native_file_upload` 和 `packaged_ui_sampling` 四个人工 evidence 未填。
- 本轮实际运行 `python scripts/verify_release_candidate.py --require-artifacts --run-dmg-ui-sampling-smoke --report-json tmp/rc-verification-packaged-ui.json`，当前 `dist/electron/Oha-Yachiyo-0.4.0-arm64.dmg` 内真实 packaged `.app` 已启动并通过 `/status`，Chromium DevTools 抽样 `#/chat`、`#/agents/agents`、`#/agents/workflows`、`#/activity-all`、`#/diagnostics`、`#/proactive-tts` 和 `#/settings/live2d` 的稳定 selector 均通过；同一份 report 自动把 `packaged_bridge_isolation` 和 `packaged_ui_sampling` 标为 `passed`。再用 `python scripts/verify_release_candidate.py --manual-checks-json tmp/rc-verification-packaged-ui.json --mark-provider-smoke-not-applicable-if-missing --write-manual-checks-draft tmp/rc-signoff-after-packaged-ui.json` 合并缺凭据 provider evidence 后，签核草稿进度为 3/6 complete，剩余项降为 3 个：`gatekeeper_first_launch`、`screen_recording_permission` 和 `chat_native_file_upload`。
- 本轮优化了失败 DMG 探针的 evidence 复用：`--run-dmg-screen-smoke` / `--run-dmg-ui-sampling-smoke` 只要已启动 DMG 内 `.app` 并让 `/status` 返回 `service=oha-yachiyo`，即使后续 screen capture 或 packaged renderer 抽样失败，report 也会写入 `bridge_ready_dmg_paths` 并把 `packaged_bridge_isolation` 自动标为 `passed`；失败的具体 gate 仍保持 failed / manual_required，不会误放行。实际重跑 `python scripts/verify_release_candidate.py --require-artifacts --run-dmg-screen-smoke --report-json tmp/rc-verification-screen-latest.json` 后，source / built artifact guards 通过，`bridge_ready_dmg_paths` 记录 `dist/electron/Oha-Yachiyo-0.4.0-arm64.dmg`，当前阻断仍是系统 `screen_capture_permission_denied`。
- 2026-06-13 又以当前 DMG 重跑 `python scripts/verify_release_candidate.py --require-artifacts --run-dmg-screen-smoke --report-json tmp/rc-verification-screen-current.json`：source release guards 和 built artifact guards 通过，DMG 内 packaged app 的 Bridge 已能响应 `/status`，失败仍只在 `/screen/current` 的 `screen_capture_permission_denied`。因此当前合并签核状态仍以 `tmp/rc-signoff-after-packaged-ui-and-chat-file.json` 为准：4/6 complete，剩余 `gatekeeper_first_launch` 与 `screen_recording_permission`。
- 本轮补强了失败 screen smoke 的签核备注复用：当 previous RC report 的 `dmg_screen_probe` 已经到达 packaged Bridge 但 `/screen/current` 失败时，`--write-manual-checks-draft` 会把短备注合并到 `screen_recording_permission.notes`，保留 DMG 路径和 `screen_capture_permission_denied` 这类错误码，但不复制一次性临时 backend 路径，也不把该项误标为 passed。实际用 `tmp/rc-signoff-after-packaged-ui-and-chat-file.json` 加 `tmp/rc-verification-screen-current.json` 重生成 `tmp/rc-signoff-current-with-screen-attempt.json` 后仍为 4/6 complete，剩余项仍是 `gatekeeper_first_launch` 与 `screen_recording_permission`。
- 本轮新增并实际运行 `--run-dmg-chat-native-file-smoke`：重新刷新 metadata、用 `.venv/bin/python scripts/build_backend.py --clean` 重建 backend、`npm --prefix apps/frontend run dist:mac` 重建 `dist/electron/Oha-Yachiyo-0.4.0-arm64.dmg` 后，`python scripts/verify_release_candidate.py --require-artifacts --run-dmg-chat-native-file-smoke --report-json tmp/rc-verification-chat-native-file.json` 已通过。该 gate 挂载 DMG、启动真实 packaged Electron app，并在显式 `OHA_YACHIYO_DESKTOP_SMOKE_MODE=1` 下让主进程 `chooseChatImages` IPC 读取脚本生成的 `packaged-native-picker-smoke.svg`；随后通过 Chromium DevTools 点击 Chat 附件按钮，验证 preview、send payload、message attachment、image viewer open/close 和 Run Detail replay handoff。report 写入 `dmg_chat_native_file_smoke.uploads`，`selected_file_count=1`、`submitted_attachment_count=1`、`run_id=main_chat_run_packaged_native_file_smoke`、`image_viewer_verified=true`、`run_detail_verified=true`、`desktop_picker_ipc_verified=true`，并把 `chat_native_file_upload` 自动标为 `passed`；正常用户运行不设置 smoke env，仍使用系统原生 file picker。
- 本轮用 `python scripts/verify_release_candidate.py --manual-checks-json tmp/rc-verification-packaged-ui.json --manual-checks-json tmp/rc-verification-chat-native-file.json --mark-provider-smoke-not-applicable-if-missing --write-manual-checks-draft tmp/rc-signoff-after-packaged-ui-and-chat-file.json` 合并 packaged UI、Chat native-file 和 provider-not-applicable evidence 后，签核草稿进度为 4/6 complete，剩余项降为 2 个：`gatekeeper_first_launch` 和 `screen_recording_permission`。此次也修复了多份 previous RC report 合并时后者 `manual_required` 覆盖前者自动 evidence 的问题。
...
- 本轮把 provider smoke helper 的 Responses list text snapshot 自测也纳入 release verifier guard：`response.output_text.done.text` 为数组时，helper 会用 final snapshot 覆盖 delta 草稿、只输出长度/finish_reason 等安全摘要且不泄露文本内容；这使真实 provider smoke 与 NativeRunEngine 的 list snapshot 合同保持同等发布守卫。
...
- 本轮继续补齐 direct group Agent 的审批拒绝 Bridge 路径：群组会话中直接点名 Agent 触发 `terminal.run` approval-required 后，经 `/ui/runs/{run_id}/approval/reject` 拒绝，Chat 会投影 cancelled Agent 消息并自动创建 `[Oha-Yachiyo 群组直接 Agent 汇总]` summary task；随后 NativeRunEngine 生成主模型 summary run，并验证 rejected Agent replay、summary Run Detail、RunEvent replay 和父消息 summary 状态同步；该测试也已加入 macOS release workflow 与 release verifier 必跑项。
...
- 本轮补强最终签核状态查询：`verify_release_candidate.py --print-manual-checks-status` 可只读加载已有 RC report、JSON draft 或 Markdown checklist，打印同一套 progress、remaining ids、next actions 和剩余自动收证命令，不运行 artifact / DMG / UI gate，也不写新文件；生成 Markdown checklist 时也会把剩余自动收证命令写入 `Remaining Automation Commands`。release packaging 文档和 release verifier required-text 已同步守卫该入口。
...
- 本轮补强最终签核 traceability：`verify_release_candidate.py` 的 RC report 现在写入 `source_revision`（git commit / short commit / dirty 状态），由上一轮 RC report 生成的 `manual-rc-checks.draft.json` 会保留 `manual_release_candidate_check_source_revisions`，最终 `--manual-checks-json` / `--manual-checks-markdown` RC report 也会继续归档这些 source revision；`--require-manual-checks-complete` 还会拒绝缺失 source revision metadata、dirty 或与当前 `source_revision.commit` 不一致的 manual evidence source revision，并写入 `manual_release_candidate_check_source_revision_findings`。release packaging 文档和 release verifier required-text 已同步守卫，避免 Gatekeeper / Screen Recording 人工 evidence 脱离具体源码版本。
- 本轮继续补强最终签核 source hygiene：`--require-manual-checks-complete` 现在会在当前 `source_revision` 可用且 dirty 时失败，并把 finding 写入 `source_revision_final_signoff_findings`；最终签核必须基于 clean git revision，避免人工 evidence 或 DMG smoke 绑定到未提交源码状态。
- 本轮继续补强 packaged artifact traceability：Bridge `/status` 现在会暴露 packaged build metadata，`--run-dmg-app-smoke`、`--run-dmg-screen-smoke` 和 `--run-dmg-ui-sampling-smoke` 会把 DMG 内真实启动的 `/status` 摘要归档到 `bridge_statuses`，包含 DMG 路径、service、version、native agent readiness 和 build metadata；`--run-dmg-chat-native-file-smoke` 会额外归档 packaged Electron app 的 `app_build_metadata`。RC gate 在当前 `source_revision.commit` 可用时会要求 packaged `build_metadata.commit` / `app_build_metadata.commit` 与之匹配，否则把对应 DMG smoke section 标为 failed，阻断 stale DMG、旧 backend 或旧 Electron app 进入最终签核。release verifier required-text 已守卫该字段和比较逻辑，避免后续 RC report 只保留源码 `source_revision`，却丢失真实 packaged artifact 对应 commit 的证据。
- 本轮实际尝试 `python scripts/verify_release_candidate.py --require-artifacts --run-dmg-screen-smoke --report-json tmp/rc-verification-screen-current.json`：DMG 内 packaged Bridge 已启动并通过 `/status`，但 `/screen/current` 因 `screen_capture_permission_denied` 失败，确认当前剩余屏幕录制项是 macOS 授权问题而非 packaged Bridge 启动问题。`manual_release_candidate_check_summary` 现在包含 `remaining_notes`，`--print-manual-checks-status` 会直接打印 screen gate 的 supporting note，避免只看到泛化 next action。
- 本轮又按当前 `a204f9b` clean source revision 重新刷新 build metadata、重建 packaged backend 和 `dist/electron/Oha-Yachiyo-0.4.0-arm64.dmg`。新的批量 RC gate `python scripts/verify_release_candidate.py --require-artifacts --check-dmg-mount --run-dmg-app-smoke --run-dmg-ui-sampling-smoke --run-dmg-chat-native-file-smoke --report-json tmp/rc-verification-a204f9bb-packaged-batch.json` 已通过，DMG mount、packaged app startup、packaged UI sampling、packaged Chat native-file smoke 和 packaged build metadata revision guards 全部通过。随后 `--run-dmg-screen-smoke --report-json tmp/rc-verification-a204f9bb-screen.json` 仍只因 `screen_capture_permission_denied` 失败，build metadata revision guard 已通过。合并 batch evidence、screen attempt 和 provider credentials unavailable 后，`tmp/rc-signoff-a204f9bb-current.json` 为 4/6 complete；最终签核预览 `tmp/rc-signoff-a204f9bb-preview.json` 的剩余项只剩 `gatekeeper_first_launch` 与 `screen_recording_permission`。
- 本轮在提交审计记录前按代码 commit `6aa67a5b` clean source revision 重新刷新 packaged build metadata、重建 `dist/backend/oha-yachiyo-backend` 与 `dist/electron/Oha-Yachiyo-0.4.0-arm64.dmg`，随后恢复 tracked `apps/frontend/public/oha-yachiyo-build.json` 开发占位，保证源码树 clean 而 packaged artifact 内 metadata 指向对应 commit。新的批量 RC gate `python scripts/verify_release_candidate.py --require-artifacts --check-dmg-mount --run-dmg-app-smoke --run-dmg-ui-sampling-smoke --run-dmg-chat-native-file-smoke --report-json tmp/rc-verification-6aa67a5b-packaged-batch.json` 已通过，DMG mount、packaged app startup、packaged UI sampling、packaged Chat native-file smoke 和 packaged build metadata revision guards 全部通过，report 的 `source_revision.dirty=false` 且 source revision findings 为空。随后 `python scripts/verify_release_candidate.py --require-artifacts --run-dmg-screen-smoke --report-json tmp/rc-verification-6aa67a5b-screen.json` 已到达 packaged Bridge，但 `/screen/current` 仍因 `screen_capture_permission_denied` 失败；合并 batch evidence、screen attempt 和 provider credentials unavailable 后，`tmp/rc-signoff-6aa67a5b-current.json` 为 4/6 complete，最终签核预览 `tmp/rc-signoff-6aa67a5b-preview.json` 只因 `gatekeeper_first_launch` 与 `screen_recording_permission` 仍为 `manual_required` 失败，`source_revision_final_signoff_findings` 与 `manual_release_candidate_check_source_revision_findings` 均为空。本审计记录提交后会产生新的 HEAD；正式 final signoff 仍需对最新 HEAD 重跑同一批处理，不能复用旧 commit 的 manual evidence source revision。
- 本轮继续把 Screen Recording 授权目标从随机 DMG mount path 收敛到稳定 app path：`--run-dmg-screen-smoke` 现在会把 DMG 内 `.app` 复制到 `tmp/rc-screen-smoke/Oha-Yachiyo-0.4.0-arm64/Oha-Yachiyo.app` 再启动，失败 report 的 `dmg_screen_probe.app_launch_paths` 与 `screen_recording_permission` supporting note 会记录该路径，方便 macOS TCC 对同一路径授权后复跑。按 commit `471c9354` 重建并刷新 RC 后，batch gate 通过，screen gate 仍只因 `screen_capture_permission_denied` 失败，合并签核仍为 4/6 complete。
- 本轮新增 `scripts/refresh_local_rc_signoff.py --write-os-evidence`：签核人完成 Gatekeeper / Screen Recording 后可生成只包含 `gatekeeper_first_launch` 与 `screen_recording_permission` 的小 JSON，并继承当前 `tmp/rc-signoff-<short-commit>-current.json` 的 `manual_release_candidate_check_source_revisions`，再作为第二个 `--manual-checks-json` 输入最终 gate，避免手工编辑整份 checklist 或丢失 source revision traceability。release packaging 文档和 release artifact verifier 已守卫该入口。按 commit `7f22f5b7` 重建并刷新 RC 后，batch gate 通过，screen gate 仍只因 `screen_capture_permission_denied` 失败，合并签核仍为 4/6 complete。
- 本轮进一步让 `scripts/refresh_local_rc_signoff.py --print-status` 在剩余 `gatekeeper_first_launch` / `screen_recording_permission` 时直接打印可复制的 `--write-os-evidence tmp/rc-signoff-<short-commit>-os-evidence.json` 命令；该命令只作为收证入口，不改变 verifier 对 `--require-manual-checks-complete` 的判定。按最新 commit `1bdb4b12` 重新刷新 build metadata、重建 packaged backend 和 `dist/electron/Oha-Yachiyo-0.4.0-arm64.dmg` 后，`tmp/rc-verification-1bdb4b12-packaged-batch.json` 通过，`tmp/rc-verification-1bdb4b12-screen.json` 已到达 packaged Bridge 但 `/screen/current` 仍因 `screen_capture_permission_denied` 失败，`tmp/rc-signoff-1bdb4b12-current.json` 为 4/6 complete，`tmp/rc-signoff-1bdb4b12-preview.json` 只剩 `gatekeeper_first_launch` 与 `screen_recording_permission`，且 source revision / manual source revision findings 均为空。
- 本轮硬化了 packaged UI sampling smoke 的等待与诊断边界：commit `bcb824d1` 先为关键 selector 加入最小等待与失败 snapshot，随后 commit `cec03482` 把等待模型改为每个 route 共享 `MIN_ROUTE_TIMEOUT_MS=15000`，并把 Python wrapper 的 DMG UI sampling 父进程 timeout 调整为默认 190s，避免子进程还在收集缺失 selector / 页面 snapshot 时被父进程提前杀掉。`node --check scripts/smoke_packaged_ui_sampling.mjs`、release candidate verifier / release artifact verifier focused tests、`scripts/verify_release_artifacts.py`、真实 `--run-dmg-ui-sampling-smoke` 和当前 HEAD 的完整本地 RC 刷新均已通过；`tmp/rc-verification-cec03482-packaged-batch.json` 为 `ok=true`，`tmp/rc-verification-cec03482-screen.json` 仍只因 `screen_capture_permission_denied` 失败，`tmp/rc-signoff-cec03482-current.json` 与 `tmp/rc-signoff-cec03482-preview.json` 继续收敛为 4/6 complete，剩余 `gatekeeper_first_launch` 与 `screen_recording_permission`。
- 本轮新增 `scripts/refresh_local_rc_signoff.py --print-os-signoff-guide`，把最后两个 OS 项的操作路径收敛成只读指南：读取当前 `tmp/rc-signoff-<short-commit>-current.json` 与 screen report，打印 DMG、稳定 Screen Recording app path、`open -R ...`、System Settings Screen Recording URL、授权后 rerun 的 `--run-dmg-screen-smoke`、`--write-os-evidence` 和最终 `--require-manual-checks-complete` 命令。该入口不会写入 evidence，也不会把 Gatekeeper / Screen Recording 标为 passed；release packaging 文档和 release artifact verifier 已守卫该入口，避免最终收证步骤再次散落。
- 本轮发现 macOS `录屏与系统录音` 中 `Oha-Yachiyo` `.app` 开关已为 on，但 `--run-dmg-screen-smoke` 仍以 `screen_capture_permission_denied` 失败，错误目标指向 packaged backend helper `Contents/Resources/backend/oha-yachiyo-backend`。因此 RC screen report 的 `dmg_screen_probe.app_launch_paths` 现在同时写入稳定 `app_path` 和 `backend_path`，`screen_recording_permission` supporting note 与 `--print-os-signoff-guide` 也会同时打印 `.app` 和 backend executable 的 `open -R` 目标，避免签核人只重复授权 `.app` 而漏掉实际执行截图的 helper 进程。
...
5. 做最终发布验收切片：
   ...
   - 当前实际产出的 `.app` / DMG 已在同一次批量 RC gate 中通过 `--check-dmg-mount`、`--run-dmg-app-smoke`、`--run-ui-smoke`、`--run-dmg-ui-sampling-smoke` 和 `--run-dmg-chat-native-file-smoke`；最终发布前剩余人工确认首次启动文案、Gatekeeper 打开路径、Screen Recording 授权，以及在具备真实 provider credentials 时运行带 `--run-provider-smoke` 的 RC gate。屏幕录制权限优先用 `--run-dmg-screen-smoke` 自动收证，失败时再回退到人工记录 System Settings 状态和截图/proactive probe 结果。
