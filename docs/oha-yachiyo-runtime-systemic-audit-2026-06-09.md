...
- Secret 持久化前清洗已覆盖 ChatSession、ChatStore、旧 chat.db 迁移、ActivityStore、RunEvent payload、Run projection、Run artifact projection、terminal/tool 输出、terminal 子进程 env 继承边界、provider/tool exception projection、标准 logging 输出、桌面后端未捕获异常输出、Bridge HTTPException detail、UI JSON error/message、provider catalog 失败缓存、ModelProfile/NativeRunEngine failure projection，以及 ModelProfile / Agent Studio 旧明文 API Key 迁移后的 raw SQLite 清理；Chat transcript 清洗不折叠空白、不截断正文。
...
- 成熟 UI flow contract 已有同步 pytest 覆盖，锁定 Chat 文本/图片发送、idempotency、停止生成、Run approval approve/reject 和 Run cancel 的 Bridge 合同。
...
- Chat 取消 late-output 已加固：慢模型返回前取消任务后，Run 可靠进入 `cancelled`，late model response 不再写 `model.output.completed` 或把 Run 覆盖回 running/completed。
- Chat 图片附件已有 live source Bridge E2E：真实 `/ui/chat/messages` 提交 image data URL attachment，ChatSession 只暴露公共 attachment URL，NativeRunEngine 传给 fake model 并完成 `model.output.completed` / `run.completed` replay。
- Chat 审批已有 live source Bridge E2E：真实 `/ui/chat/messages` 触发 `workspace.write_patch` approval，`/ui/runs/{run_id}/approval/approve` 批准后实际修改 workspace 文件，恢复模型并完成 `agent.tool.approval_required` / `agent.tool.approval_approved` / approved `agent.tool.call` / `model.output.completed` / `run.completed` replay。
...
- 主聊天工具审批等待已有 Chat API 级投影兜底：RUNNING Task 可通过当前 runtime service 的 Task↔Run link 投影 `approval_required` 到 ChatSession metadata、ActivityStore 和 `approval_count`，审批恢复后会清空旧 `pending_approval`。
...
- 主聊天自动委派已有 TaskRunner 级集成回归：真实 TaskRunner / NativeAgentExecutor / NativeRunEngine 可解析 `run_oha_agent` directive、创建 delegation Agent Run、记录 ActivityStore 委派活动、把 delegated result 回填给主模型并完成最终 ChatSession 回复；delegated Run 结束后创建主模型 follow-up summary Task 已有真实 NativeRunEngine Run projection 回归；自动委派 summary 路径已修正为同时使用当前注入 runtime service 与 runtime activity store，不再隐式打开全局 NativeRunEngine / ActivityStore。
- 群聊派活已有 ChatAPI + 真实 NativeRunEngine 级集成回归：runtime 注入 `AgentRuntimeService` 后，主模型 `oha.group_dispatch` 输出会创建真实 Agent Run，群组 upstream 进入 Agent context，Agent 完成后回写群聊消息并创建主模型群总结 Task；群聊直接点名 Agent 的路径也已有 TaskRunner + 真实 NativeRunEngine 回归，覆盖 direct Agent Run 完成后创建主模型整理 Task，以及工具审批拒绝/取消后继续创建主模型整理 Task，并继续通过 `NativeAgentExecutor` 生成 main_chat_run / RunEvent replay / ChatSession 投影；ChatAPI 的 Agent/Workflow/Run 状态读取入口现在统一经 `_agent_runtime_service()`，避免成熟业务路径直接绕过 runtime 注入边界。
...
- Agent Studio Run Detail 已接入 `/runs/{run_id}/events` replay API，Execution 区优先展示 replayable RunEvent facts，选中 Run 的状态/更新时间/timeline 变化时会刷新 replay，支持按 `after_sequence` 继续加载更多 replay facts，常见 RunEvent facts 已映射为可读标题/语气，保留旧 timeline 回退；Run Detail 的 Workflow 子 Agent 审批桥接已补 source-level guard，锁定父 Workflow 选中态、子 Run approve/reject、子 Run 打开和审批后父 Run 刷新链路，并有 route contract 覆盖。
...
- 模型输出 durable persistence 已有 batched completed-event 回归、dict-style stream iterator delta 合并压力测试、OpenAI SDK object-style content chunk 合并回归、OpenAI-style streaming tool_call delta 合并回归、OpenAI-compatible SSE stream parser / NativeRunEngine `stream=True` contract 回归、fake HTTP provider SSE 闭环回归、fake HTTP provider coalesced/split SSE frame 闭环回归、fake HTTP provider SSE tool-call 与 split-frame SSE tool-call 闭环回归，以及 fake HTTP provider SSE error frame 失败/清洗闭环回归；现在也有 opt-in `scripts/smoke_openai_compatible_stream.py` 可用真实 provider 做 streaming / tool-call smoke，支持要求流式文本内容、指定工具名和 tool-call arguments substring，脚本自身已有 fake transport 回归和错误 secret 清洗回归，且摘要不打印 raw tool arguments；实际凭据环境下的真实外部 provider 联调仍需做。
...
- release/alpha/stable 源码级 guard、release-facing verifier、packaged app resources scan 与 release 目录 binary-safe artifact scan 已覆盖；workflow 会排除本地 `node-pty/build` native artifact 并只打包 clean prebuilds；本地 unsigned `.app` / DMG 产物已验证不包含旧产品身份 token。Electron Framework 内部自带的通用 `Hermes` 字符串不属于本项目产品身份或执行内核残留。
