...
- Bubble / Live2D launcher 现在锁定更多浏览器 E2E 断言点：Bubble button/status/summary、Live2D stage/canvas/fallback/resource hint/reply/quick input，以及 hidden session summary probe 的 latest reply/status/recent sessions；后续可稳定验证群聊/自动委派/会话总结进入 launcher 与 Live2D 快速输入路径。
