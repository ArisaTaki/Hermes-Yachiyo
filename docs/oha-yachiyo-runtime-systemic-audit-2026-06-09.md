...
2. 做 release candidate `.app` 内跨页面 packaged UI 抽样：
   - Chat UI 图片附件的自动链路已有 source Bridge E2E、HTTP route roundtrip、TaskRunner image roundtrip、RunEvent replay 和本地 Electron smoke 的桌面 `chooseChatImages` API / hidden input fallback / CDP file input / 附件预览 / 提交闭环；剩余只需人工确认 packaged app 的 OS file picker 弹窗本身。
