...
- packaged app verifier 现在还会扫描 `Resources/app.asar` 原始字节，确认 Chat 图片上传/预览/消息附件/image viewer open-close、群聊创建/Agent 成员选择/Activity Run Detail handoff、Diagnostics 本地截图摘要、Live2D settings 资源导入、Bubble / Live2D session summary、Live2D quick input、Proactive TTS settings/status/result、Chat 取消、Chat 审批 approve/reject/reveal/open-run、Chat completed message Run Detail、Agent Studio agents 定义 CRUD、Agent Studio Skill Library sync/install/update/delete、Run Detail replay/artifact/rerun、Workflow child approval approve/reject/cancel/open-run 和 Workflow Studio 编辑/save-and-run/Run Detail 投影的关键 UI selector 进入发布产物，避免后续 Browser/Desktop E2E 所依赖的成熟 UI 入口在 build/packaging 阶段被剥掉。
- release artifact verifier 单测现在会从 release workflow 的十四个 Electron UI smoke 中提取显式 `data-testid`，并要求这些 selector 全部进入 `PACKAGED_UI_E2E_REQUIRED_SELECTORS`，避免后续 smoke 继续新增成熟 UI 入口时漏同步 packaged app selector gate。
- release workflow smoke 现在会在 packaging 前直接运行 Chat 图片附件、Chat 取消、Chat 审批、Chat delegated summary、群聊 summary、Activity feed/detail、Diagnostics local screenshot、Live2D settings、launcher session summary、Proactive TTS、Agent Studio agents 定义 CRUD、Agent Studio Skill Library、Agent Run Detail replay / Workflow child approval 和 Workflow save-and-run 十四个 Electron UI smoke；release verifier 会阻断 workflow 丢失这些关键 smoke。
