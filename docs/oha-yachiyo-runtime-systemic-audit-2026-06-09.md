...
- 可重复本地 UI smoke `node scripts/smoke_agent_run_detail_ui.mjs` 会启动 mock Bridge、Vite dev server 和 Electron BrowserWindow，加载 `#/agents/{run_id}`，seed 一个 completed Native Agent Run、TaskRunLink 投影、RunGroup 和 3 条 `/runs/{run_id}/events` replay facts，断言 Run Detail article、Task、Result 和 `agent-run-detail-execution-event` 的 `data-run-event` / sequence / run-id 属性均指向同一个 Run；source-level pytest guard 锁定该 smoke 必须走 replay route 和稳定 DOM 属性。
