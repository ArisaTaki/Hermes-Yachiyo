...
- 桌面 `.app` 已实际启动并验证 bridge；主要 UI 页面已有静态入口 guard、浏览器级 route smoke、source Browser 按钮级交互 smoke、DMG 内 packaged app startup smoke、十九个本地 Electron UI smoke 和真实 DMG packaged renderer 抽样。剩余 UI 人工重点收敛到真实 OS file picker 弹窗、Gatekeeper/Finder 首启路径和 Screen Recording 授权。
...
5. 做最终发布验收切片：
   ...
   - 当前实际产出的 `.app` / DMG 已在同一次批量 RC gate 中通过 `--check-dmg-mount`、`--run-dmg-app-smoke` 和 `--run-ui-smoke`；最终发布前剩余人工确认首次启动文案、Gatekeeper 打开路径、原生图片 file picker，以及在具备真实 provider credentials 时运行带 `--run-provider-smoke` 的 RC gate。packaged UI 抽样优先用 `--run-dmg-ui-sampling-smoke` 自动收证；屏幕录制权限优先用 `--run-dmg-screen-smoke` 自动收证，失败时再回退到人工记录 System Settings 状态和截图/proactive probe 结果。
