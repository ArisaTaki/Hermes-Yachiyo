...
- Chat 图片附件的大图预览 modal / stage / close action 也已补稳定 `data-testid`，后续浏览器 E2E 可以继续断言“点击消息附件 → 预览打开 → 关闭预览”的交互闭环。
...
- packaged app verifier 现在还会扫描 `Resources/app.asar` 原始字节，确认 Chat 图片上传/预览/消息附件、Chat 取消、Chat 审批 approve/reject/open-run、Run Detail replay、Workflow child approval approve/reject/cancel/open-run 和 Workflow save-and-run 的关键 `data-testid` selector 进入发布产物，避免后续 Browser/Desktop E2E 所依赖的成熟 UI 入口在 build/packaging 阶段被剥掉。
