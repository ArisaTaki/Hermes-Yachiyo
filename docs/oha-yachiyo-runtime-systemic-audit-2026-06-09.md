...
- release candidate report 现在还会写入结构化 `manual_release_candidate_check_statuses`，把 `gatekeeper_first_launch`、`packaged_bridge_isolation`、`screen_recording_permission`、`chat_native_file_upload`、`packaged_ui_sampling` 和 `real_provider_smoke` 六个无法完全自动化的最终验收项标成 `manual_required`，并记录每项 release signoff 前需要附上的 evidence；旧的 `manual_release_candidate_checks` 字符串列表保留兼容。`--write-manual-checks-template` 可生成带 `evidence_prompt` 的项目内 JSON 模板，release workflow 会把 `release/manual-rc-checks.template.json` 随 DMG/RC report 一起上传；`--manual-checks-json` 可从项目内 JSON 合并人工 evidence，支持 `manual_required` / `passed` / `failed` / `not_applicable`，未知 id、重复 id、非法 status、缺 evidence 或显式 failed 都会让 RC gate 失败并写入 `manual_release_candidate_check_findings`。release verifier 会阻断 RC verifier、release workflow 或 release packaging 文档丢失这些字段、关键 id、模板输出或 evidence 输入入口。
