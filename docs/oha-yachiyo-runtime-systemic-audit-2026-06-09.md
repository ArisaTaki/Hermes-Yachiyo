...
- message-level SSE tool-call、multiline `data:` SSE tool-call、split-frame SSE tool-call、indexless / stable-id 归并 SSE tool-call、legacy `delta.function_call` 与 Responses-style NativeRunEngine 主循环回归已纳入 macOS release workflow smoke，release verifier 会阻断 workflow 丢失这些测试，避免发布门禁只覆盖普通 delta 风格 Chat Completions SSE 而漏掉 message-level、多行/分片 transport、缺 `index` 的 gateway 形态、旧式 `function_call` 或 Responses 风格 tool-call stream。
