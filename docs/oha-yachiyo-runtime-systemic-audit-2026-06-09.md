...
- release workflow 的 opt-in 真实 provider 文本流 smoke 现在要求 `finish_reason=stop`，tool-call smoke 要求 `finish_reason=tool_calls`；release verifier 会阻断丢失任一断言的 workflow，避免真实 provider 未完成标准结束协议时误过发布门禁。
