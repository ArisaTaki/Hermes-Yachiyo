...
- Chat 图片附件的大图预览 modal / stage / close action 已补稳定 `data-testid`，本地 Electron smoke 现在也断言“点击消息附件 → 预览打开 → 关闭预览”的交互闭环；同一 smoke 还锁定 composer preview 的 id/mime/size/width/height、提交给 `/ui/chat/messages` 的 client attachment id/mime/size/dimensions/data URL，以及消息附件 DOM 的 id/kind/mime/name/size 元数据，避免图片 UI 上传路径丢失附件 handoff 信息。
