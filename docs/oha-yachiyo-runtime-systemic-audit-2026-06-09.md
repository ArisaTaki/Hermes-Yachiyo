...
- 成熟 UI flow contract 已有同步 pytest 覆盖，锁定 Chat 文本/图片发送、idempotency、停止生成、会话列表/搜索/加载/清空/丢弃空会话/删除、Agent Studio 定义 list/create/update/delete、Workflow Studio 定义 list/create/update/delete、Run / RunGroup 列表与详情、Run artifact、rerun、delete、approval approve/reject 和 Run cancel 的 Bridge 合同。
