...
- packaged app verifier 现在还会扫描 `Resources/app.asar` 原始字节，确认 Chat 图片上传/预览/消息附件、Chat 取消、Chat 审批、Run Detail replay、Workflow child approval approve/reject/cancel/open-run 和 Workflow save-and-run 的关键 `data-testid` selector 进入发布产物，避免后续 Browser/Desktop E2E 所依赖的成熟 UI 入口在 build/packaging 阶段被剥掉。
- release workflow smoke 现在会在 packaging 前直接运行 Chat 图片附件、Chat 取消、群聊 summary、launcher session summary、Agent Run Detail replay / Workflow child approval 和 Workflow save-and-run 六个 Electron UI smoke；release verifier 会阻断 workflow 丢失这些关键 smoke。
