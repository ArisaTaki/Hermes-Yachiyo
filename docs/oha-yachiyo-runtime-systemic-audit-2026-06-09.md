...
- 可重复本地 UI smoke `node scripts/smoke_agent_run_detail_ui.mjs` 会启动 mock Bridge、Vite dev server 和 Electron BrowserWindow，加载 `#/agents/{run_id}`，seed 一个 completed Native Agent Run、TaskRunLink 投影、RunGroup、artifact 和 201 条 `/runs/{run_id}/events` replay facts；首屏断言 Run Detail article、Task、Result、前 200 条 `agent-run-detail-execution-event` 的 `data-run-event` / sequence / run-id 属性和 `agent-run-detail-load-more-events`，点击加载更多后再断言第 201 条 completed replay 合并进同一个 Run 且 load-more 消失，并点击 `agent-run-detail-artifact` 验证 `/ui/runs/{run_id}/artifacts/{path}` preview 渲染；随后点击 `agent-run-detail-rerun`，验证 `/ui/runs/{run_id}/rerun` 返回的新 Run 自动打开，并通过 `/runs/{rerun_id}/events` 展示 `run.rerun.started` / completed replay。source-level pytest guard 锁定该 smoke 必须走 replay route、artifact route、rerun route、稳定 DOM 属性和分页 UI。
...
- packaged app verifier 现在还会扫描 `Resources/app.asar` 原始字节，确认 Chat 图片上传/预览/消息附件、Chat 审批、Run Detail replay 和 Workflow save-and-run 的关键 `data-testid` selector 进入发布产物，避免后续 Browser/Desktop E2E 所依赖的成熟 UI 入口在 build/packaging 阶段被剥掉。
- release workflow smoke 现在会在 packaging 前直接运行 `node scripts/smoke_chat_image_attachment_ui.mjs` 和 `node scripts/smoke_agent_run_detail_ui.mjs`；release verifier 会阻断 workflow 丢失 Chat 图片 Electron UI smoke 或 Agent Run Detail replay Electron UI smoke。
