...
- release artifact verifier 单测现在也要求十九个 Electron UI smoke 脚本全部进入 `RELEASE_WORKFLOW_SMOKE_TEST_REQUIRED_TEXT`，并显式断言 Workflow management/delete smoke 的 release workflow 缺失会报错，避免 smoke 已纳入发布流程但 verifier 负例遗漏。
