...
- Chat 发送后的 message attachment list / attachment item 现在也有稳定 `data-testid`，attachment item 同时暴露 `data-attachment-id/name/kind/mime`，与 composer 上传预览/移除 selector 配套，后续真实 file upload 浏览器 E2E 可以稳定断言“选择文件 → 预览 → 发送 → 消息附件渲染”闭环。
