...
- 可重复本地 UI smoke `node scripts/smoke_chat_image_attachment_ui.mjs` 会启动 mock Bridge、Vite dev server 和 Electron BrowserWindow，加载 Chat 页面，在页面上下文构造真实 `File` 并通过 hidden `chat-image-file-input` 的 `DataTransfer.files` 派发 `change`，断言 composer preview、`/ui/chat/messages` 发送 payload、message attachment item 和 image viewer modal；本轮已实际运行通过。该 smoke 不依赖 dev-only image event，覆盖 `input.files → onChange → addImageFiles() → FileReader → preview → send` 路径，并由 source-level pytest guard 锁定不能退回 `oha-chat-e2e-add-image` 注入。
