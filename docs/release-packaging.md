...
9. 对 `release/` 目录执行 binary-safe release artifact scan，确认最终 DMG、JSON、checksum 和 notes 没有旧产品身份或旧执行内核 token，并校验每个 DMG 的 `.sha256` 文件和 latest JSON 的 `dmg_name` / `sha256` 均与同目录 DMG 内容一致。
