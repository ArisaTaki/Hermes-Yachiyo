...
该命令会运行 source-level release guard，默认执行数据分析 sample dataset -> Markdown/CSV/HTML/PNG artifact readback smoke，并对已生成的 `dist/backend`、`dist/electron` 和 `release` 执行 binary/package verifier。需要确认 DMG 内真实 `.app` 也可扫描时运行：
