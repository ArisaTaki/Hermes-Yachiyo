"""Read-only installed-app discovery after a trusted name-resolution miss."""
...
from apps.shell.agent.runtime.recovery_policies import app_resolution_source
from apps.shell.agent.runtime.tool_outcomes import OutcomeStatus

_ACTION = "resolve_app_identity"
_STRATEGY_ID = "resolve-app-identity"
_RECOVERY_HINT = "app_resolution_failed"
_SOURCE_CAPABILITIES = ("desktop.app_control",)
_REQUIRED_CAPABILITIES = ("desktop.app_discovery",)
_DISCOVERY_TOOL = "desktop.list_apps"


class DesktopAppResolutionAdapter:
    """List candidates once and leave all open/focus decisions to the model."""
    ...
    def supports(self, context: RecoveryActionContext) -> bool:
        ...
        return bool(
            ...
            and app_resolution_source(outcome) is not None
        )
    ...
    def execute(self, context: RecoveryActionContext) -> RecoveryActionResult:
        ...
        source = app_resolution_source(context.source_outcome)
        if source is None:
            return RecoveryActionResult.not_handled(reason="source_unavailable")
        request = {
            "protocol": "json_fallback",
            "tool": _DISCOVERY_TOOL,
            "tool_call_id": _scoped_call_id("app-resolution-list", context),
            "input": {"query": source.query, "limit": 20},
    ...
    def reconcile_completed_attempt(
        ...
        matching_requests = [
            ...
            and str(request["input"].get("query") or "").strip() == source.query
        ]
        ...
        data = (
            ...
            if isinstance(discovery_result.get("data"), Mapping)
            else {}
        )
        candidates = data.get("apps")
        if not isinstance(candidates, list):
            candidates = data.get("items")
        if isinstance(candidates, list) and not candidates:
            return RecoveryActionResult.await_user(
                reason="app_discovery_no_match",
                attempts=(batch,),
            )
...
__all__ = ["DesktopAppResolutionAdapter"]
