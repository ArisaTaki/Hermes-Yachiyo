...
class RuntimeCustomApiAgentLoop:
    ...
    def _system_message(
        ...
        *,
        ...
        desktop_tool_guidance = (
            ...
            "when they are allowed. When both app.open/app.focus and foreground desktop operation tools are allowed, "
            "prefer split execution: first discover or open/focus the app, then call desktop.safe_shortcut, "
            "desktop.click_ui_element, desktop.type_into_ui_element, desktop.safe_type_text, desktop.safe_key, "
            "desktop.safe_scroll, desktop.safe_click, or desktop.submit_foreground, then verify. Treat app.*_and_* "
            "tools as compatibility fallbacks for runtimes that do not expose the split path, not as the default "
            "planning model. For explicit media requests, treat the media app as a discoverable "
            ...
            "app.open/app.focus followed by desktop.safe_shortcut when those tools are allowed; use "
            "app.open_and_safe_shortcut or app.focus_and_safe_shortcut only as compatibility fallbacks. "
            "map app open/focus followed by explicit safe navigation keys, scroll, single-click coordinates, "
            "visible-label clicks, or visible-field typing to app.open/app.focus followed by desktop.safe_key, "
            "desktop.safe_scroll, desktop.safe_click, desktop.click_ui_element, or desktop.type_into_ui_element "
            "when available; use app.*_and_safe_key, app.*_and_safe_scroll, app.*_and_safe_click, "
            "app.*_and_click_ui_element, or app.*_and_type_into_ui_element only when the split path is unavailable. "
    ...
    def _runtime_planner_guidance(
        ...
        return (
            ...
            "Follow the planned tool path in order when the named tools are allowed; do not replace a split "
            "app.open/app.focus plus desktop.* plan with older app.*_and_* shortcut tools unless a required split "
            "step is unavailable. "
...
def _desktop_content_followup_payload(
    ...
    *,
    ...
    content_snapshot = _latest_desktop_content_snapshot(timeline, observation_tools)
    if content_snapshot:
...
def _latest_desktop_content_snapshot(
    timeline: list[dict[str, Any]],
    ...
    for event in reversed(timeline):
        ...
        if tool_name == "desktop.ui_elements":
            return _desktop_ui_elements_content_snapshot(result, input_preview)
        if tool_name == "screen.capture":
            return _screen_capture_content_snapshot(result, input_preview)
    return {}


def _desktop_ui_elements_content_snapshot(
    result: dict[str, Any],
    ...
    lines, truncated = _desktop_ui_element_text_lines(elements)
    app_name = str(
        data.get("app_name")
...
def _screen_capture_content_snapshot(
    result: dict[str, Any],
    input_preview: dict[str, Any],
) -> dict[str, Any]:
    data = result.get("data") if isinstance(result.get("data"), dict) else {}
...
def _desktop_ui_element_text_lines(
    elements: list[Any],
    *,
    max_items: int = 32,
    max_chars: int = 3200,
    ...
    for element in elements:
        if not isinstance(element, dict):
            continue
        text = _desktop_ui_element_text(element)
        if not text:
            continue
...
def _desktop_ui_element_text(element: dict[str, Any]) -> str:
    values = [
        _clean_desktop_content_text(element.get("value")),
        _clean_desktop_content_text(element.get("name")),
        _clean_desktop_content_text(element.get("description")),
    ]
    for value in values:
        if _useful_desktop_content_text(value):
            return value
    return ""


def _clean_desktop_content_text(value: Any) -> str:
...
def _useful_desktop_content_text(value: str) -> bool:
    text = str(value or "").strip()
    if len(text) < 2:
        return False
