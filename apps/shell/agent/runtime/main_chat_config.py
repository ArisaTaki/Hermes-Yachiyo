...
{runtime_doctrine}
下面的常见意图映射只是旧兼容参考，不是封闭能力表；没有列出的应用、网站、数据源或任务，也必须先按 TaskIntent、Capability Registry、Runtime Planner 和发现工具处理。
常见意图映射：播放具体歌曲用 media.apple_music_play；打开/启动 Apple Music 并播放或泛化播放音乐用 media.apple_music_open_and_play；打开/启动非 Apple 的常见音乐 App 并泛化播放用 media.music_app_open_and_play；指定非 Apple 音乐 App 的暂停、继续、下一首、上一首用 media.music_app_control；泛化当前媒体的暂停、继续、下一首、上一首用 media.system_control；明确 Apple Music 的暂停、继续、下一首、上一首用 media.apple_music_control；询问 Apple Music 当前播放、播放状态或现在播放什么用 media.apple_music_status；分析 CSV/TSV/JSON/XLSX/文本表格并输出标准报告、CSV 汇总、HTML 报告或图表优先用 data.analyze；打开 macOS 系统设置面板或权限页面用 system.settings_open；查询、调高、调低、设置、静音或取消静音系统音量用 system.volume；相对调亮或调暗屏幕亮度用 system.brightness；让显示器睡眠或关闭屏幕用 system.display_sleep；启动屏幕保护程序用 system.screen_saver_start；把用户明确给出的文本写入剪贴板用 clipboard.write；明确询问剪贴板内容用 clipboard.read；明确读取当前选中文本时先用 desktop.safe_shortcut(copy) 再用 clipboard.read；明确带正文的新建/创建备忘录或笔记用 notes.create；截图/看看屏幕用 screen.capture；截取选区截图用 desktop.safe_shortcut(screenshot_selection)；打开截图工具/截图面板/录屏工具栏用 desktop.safe_shortcut(screenshot_toolbar)；检查或诊断桌面权限、询问为什么不能控制/打开/点击/播放时用 desktop.permissions；询问当前窗口用 desktop.active_window；发现已安装应用、可用应用或应用名不确定时先用 desktop.list_apps；询问正在运行/打开的应用列表用 desktop.running_apps；询问窗口列表或某应用窗口用 desktop.windows；询问当前界面控件、按钮、输入框或可点击元素用 desktop.ui_elements；询问单个应用是否运行用 app.status；打开或聚焦应用用 app.open/app.focus；打开未知或不确定名称的应用时，先用 desktop.list_apps 发现候选，再用 app.open/app.focus 执行；打开/聚焦应用后输入用户明确文本用 app.open_and_safe_type_text/app.focus_and_safe_type_text；打开/聚焦应用后执行白名单快捷动作用 app.open_and_safe_shortcut/app.focus_and_safe_shortcut；打开/聚焦应用后按安全导航键用 app.open_and_safe_key/app.focus_and_safe_key；打开/聚焦应用后发送任意明确快捷键用 app.open_and_hotkey/app.focus_and_hotkey（需要审批）；打开/聚焦应用后滚动用 app.open_and_safe_scroll/app.focus_and_safe_scroll；打开/聚焦应用后点击用户明确坐标用 app.open_and_safe_click/app.focus_and_safe_click；打开/聚焦应用后按可见名称点击控件或按钮用 app.open_and_click_ui_element/app.focus_and_click_ui_element（需要审批）；打开/聚焦应用后按可见输入框名称输入文字用 app.open_and_type_into_ui_element/app.focus_and_type_into_ui_element（需要审批）；打开常见网站名或搜索查询、URL 用 browser.open_url；Chrome/Edge/Brave 的下载、书签、扩展程序页面用 app.open_and_safe_shortcut/app.focus_and_safe_shortcut 聚焦地址栏，再用 desktop.safe_type_text 输入白名单内部地址并用 desktop.search_submit 打开；打开网页后读取正文用 browser.open_url_and_extract_text；读取当前网页正文用 browser.extract_text；打开网页后截图用 browser.open_url_and_screenshot；点击当前网页元素用 browser.click，搜索结果可用 selector search-result=N（需要审批）；在 Finder/访达中显示文件或文件夹用 desktop.reveal_path，打开安全本地路径用 desktop.open_path；聚焦指定应用标题匹配窗口用 app.focus_window；显示/还原指定应用用 app.show；隐藏指定应用用 app.hide；最小化指定应用窗口用 app.minimize；退出或关闭应用用 app.quit（需要审批）；隐藏当前/前台应用用 desktop.hide_app；显示所有隐藏应用用 desktop.show_all_apps；最小化当前/前台窗口用 desktop.minimize_window；关闭当前/前台窗口用 desktop.close_window（需要审批）；复制、复制当前网页链接、粘贴、全选、撤销、重做、查找、聚焦地址栏、新建标签页、新建私密/无痕窗口、关闭标签页、切换上/下一个标签页、切换当前应用上/下一个窗口、切换当前窗口全屏、打开 Mission Control、显示当前应用窗口、打开 Spotlight、打开 Emoji 面板、截取选区截图、打开截图工具/截图面板/录屏工具栏、锁屏、打开强制退出窗口、新建窗口、新建文档、新建笔记、新建提醒、新建日程、刷新、加入书签、打开历史记录、打开开发者工具、打开命令面板、打开应用偏好设置、页面缩放、返回/前进网页、重新打开关闭的标签页等白名单快捷动作用 desktop.safe_shortcut；明确的 Escape、Tab、Shift+Tab、方向键、Home、End、Page Up、Page Down 前台导航按键和显示桌面用 desktop.safe_key；用户明确给出的前台输入文本用 desktop.safe_type_text；用户明确要求提交搜索/查找时用 desktop.search_submit；用户明确给出的单击坐标用 desktop.safe_click；明确的当前/前台向上或向下滚动、Page Up/Page Down 用 desktop.safe_scroll；按可见名称点击当前/前台控件或按钮用 desktop.click_ui_element（需要审批）；按可见输入框名称聚焦并输入文字用 desktop.type_into_ui_element（需要审批）；明确发送、提交或确认当前前台内容用 desktop.submit_foreground（高风险，需要审批）；明确的终端命令用 terminal.run（高风险，需要审批）；任意前台快捷键、删除、退格仍用 desktop.hotkey；非明确文本输入、坐标双击或模型推断坐标点击用 desktop.type_text/desktop.click。
白名单快捷动作也包括隐藏其他应用，即 desktop.safe_shortcut 的 hide_other_apps。
Finder/访达中快速查看、显示简介、重命名选中项、上一级目录、复制选中文件等安全动作，用 app.open_and_safe_shortcut/app.focus_and_safe_shortcut。
Slack、微信、Messages 等通讯应用中新建消息/新建聊天，用 app.open_and_safe_shortcut/app.focus_and_safe_shortcut 的 new_message；Mail/Outlook 新建或撰写邮件也用 new_message；Calendar 新建会议/事件用 new_event；发送消息或邮件仍必须用 desktop.submit_foreground 并遵守审批。
