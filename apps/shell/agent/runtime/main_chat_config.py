...
常见意图映射：播放具体歌曲用 media.apple_music_play；打开/启动 Apple Music 并播放或泛化播放音乐用 media.apple_music_open_and_play；暂停、继续、下一首、上一首用 media.apple_music_control；查询、调高、调低、设置、静音或取消静音系统音量用 system.volume；把用户明确给出的文本写入剪贴板用 clipboard.write；截图/看看屏幕用 screen.capture；检查或诊断桌面权限、询问为什么不能控制/打开/点击/播放时用 desktop.permissions；询问当前窗口用 desktop.active_window；询问正在运行/打开的应用列表用 desktop.running_apps；询问窗口列表或某应用窗口用 desktop.windows；询问当前界面控件、按钮、输入框或可点击元素用 desktop.ui_elements；询问单个应用是否运行用 app.status；打开或聚焦应用用 app.open/app.focus；打开/聚焦应用后输入用户明确文本用 app.open_and_safe_type_text/app.focus_and_safe_type_text；打开/聚焦应用后执行白名单快捷动作用 app.open_and_safe_shortcut/app.focus_and_safe_shortcut；打开/聚焦应用后按安全导航键用 app.open_and_safe_key/app.focus_and_safe_key；打开/聚焦应用后滚动用 app.open_and_safe_scroll/app.focus_and_safe_scroll；打开/聚焦应用后点击用户明确坐标用 app.open_and_safe_click/app.focus_and_safe_click；打开/聚焦应用后按可见名称点击控件或按钮用 app.open_and_click_ui_element/app.focus_and_click_ui_element（需要审批）；打开/聚焦应用后按可见输入框名称输入文字用 app.open_and_type_into_ui_element/app.focus_and_type_into_ui_element（需要审批）；打开常见网站名或搜索查询、URL 用 browser.open_url；打开网页后读取正文用 browser.open_url_and_extract_text；打开网页后截图用 browser.open_url_and_screenshot；在 Finder/访达中显示文件或文件夹用 desktop.reveal_path，打开安全本地路径用 desktop.open_path；聚焦指定应用标题匹配窗口用 app.focus_window；显示/还原指定应用用 app.show；隐藏指定应用用 app.hide；最小化指定应用窗口用 app.minimize；退出或关闭应用用 app.quit（需要审批）；隐藏当前/前台应用用 desktop.hide_app；最小化当前/前台窗口用 desktop.minimize_window；关闭当前/前台窗口用 desktop.close_window（需要审批）；复制、粘贴、全选、撤销、重做、查找、新建标签页、刷新等白名单快捷动作用 desktop.safe_shortcut；明确的 Escape、Tab、方向键、Home、End、Page Up、Page Down 前台导航按键用 desktop.safe_key；用户明确给出的前台输入文本用 desktop.safe_type_text；用户明确给出的单击坐标用 desktop.safe_click；明确的当前/前台向上或向下滚动、Page Up/Page Down 用 desktop.safe_scroll；按可见名称点击当前/前台控件或按钮用 desktop.click_ui_element（需要审批）；按可见输入框名称聚焦并输入文字用 desktop.type_into_ui_element（需要审批）；明确发送、提交或确认当前前台内容用 desktop.submit_foreground（高风险，需要审批）；任意前台快捷键、删除、退格仍用 desktop.hotkey；非明确文本输入、坐标双击或模型推断坐标点击用 desktop.type_text/desktop.click。
