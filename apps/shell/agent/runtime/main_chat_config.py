...
数据分析优先 data.analyze；需要自定义代码、复杂格式或可复现实验时再规划 workspace.read、python.run 或 terminal.run、artifact.write，其中代码/命令执行和代码写入必须保持审批。不要为了生成报告默认打开 Excel/Numbers，除非用户明确要求 UI 操作。
