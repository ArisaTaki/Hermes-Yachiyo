...
_FIELD_POLICIES: dict[str, tuple[EffectType, str]] = {
    ...
    "tray_enabled":    (EffectType.REQUIRES_APP_RESTART,
                        "托盘设置将在下次启动时生效"),
    # Window mode
    "window_mode.width":                (EffectType.REQUIRES_MODE_RESTART,
                                         "Window 模式尺寸将在下次进入该模式时生效"),
    "window_mode.height":               (EffectType.REQUIRES_MODE_RESTART,
                                         "Window 模式尺寸将在下次进入该模式时生效"),
    "window_mode.recent_sessions_limit": (EffectType.IMMEDIATE,
                                          "Window 模式最近会话概览已更新"),
    "window_mode.recent_messages_limit": (EffectType.IMMEDIATE,
                                          "Window 模式最近消息概览已更新"),
    "window_mode.open_chat_on_start":   (EffectType.REQUIRES_MODE_RESTART,
                                         "Window 模式启动行为将在下次进入该模式时生效"),
    "window_mode.show_runtime_panel":   (EffectType.IMMEDIATE,
                                         "Window 模式总控面板已更新"),
    "window_mode.show_mode_overview":   (EffectType.IMMEDIATE,
                                         "Window 模式模式概览已更新"),
    # Bubble mode
    ...
    "live2d_mode.model_name":         (EffectType.IMMEDIATE,
                                       "模型名称已更新"),
    "live2d_mode.model_path":         (EffectType.IMMEDIATE,
                                       "模型路径已更新，已重新校验"),
