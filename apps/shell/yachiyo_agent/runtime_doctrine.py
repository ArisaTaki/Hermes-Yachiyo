...
YACHIYO_RUNTIME_DOCTRINE = (
    ...
    "structured tools over raw shell: file organization should use file.organize when available, and code/data "
    "tasks should use python.run or terminal.run only when concrete code or commands are required. Treat mounted Skills as execution manuals for specialized work: read the relevant Skill, then "
...
YACHIYO_DAILY_ENTRYPOINT_OPERATING_MANUAL = (
    ...
    "and simple chart artifacts; use workspace.read + python.run/terminal.run + artifact.write only when the analysis needs "
    ...
    "plan needs UI inspection. For code tasks, inspect the workspace before code or shell execution; use python.run/terminal.run "
