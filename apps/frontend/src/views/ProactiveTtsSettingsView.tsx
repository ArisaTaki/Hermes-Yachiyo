...
function buildGsvSetupTerminalCommand(workdir: string, serviceCommand: string, projectUrl?: string): string {
  ...
  return [
    ...
    'set -e',
    'HOST_ARCH="$(uname -m)"',
    'if [ "$(sysctl -in hw.optional.arm64 2>/dev/null || echo 0)" = "1" ]; then HOST_ARCH="arm64"; fi',
    'ROSSETTA_TRANSLATED="$(sysctl -in sysctl.proc_translated 2>/dev/null || echo 0)"',
    'if [ "$HOST_ARCH" = "arm64" ] && [ "$ROSSETTA_TRANSLATED" = "1" ]; then',
    '  echo "检测到当前终端运行在 Rosetta 翻译层；将优先选择 arm64 Homebrew/Python。"',
    'fi',
    'run_host_arch() {',
    ...
    '  BREW_BIN="/opt/homebrew/bin/brew"',
    'fi',
    'if [ -n "$BREW_BIN" ]; then eval "$(run_host_arch "$BREW_BIN" shellenv)"; fi',
    'if [ -n "$BREW_BIN" ]; then',
    '  echo "检查 Homebrew 依赖：git ffmpeg mecab python@3.11 unzip"',
    '  run_host_arch "$BREW_BIN" list git >/dev/null 2>&1 || run_host_arch "$BREW_BIN" install git',
    '  run_host_arch "$BREW_BIN" list ffmpeg >/dev/null 2>&1 || run_host_arch "$BREW_BIN" install ffmpeg',
    '  run_host_arch "$BREW_BIN" list mecab >/dev/null 2>&1 || run_host_arch "$BREW_BIN" install mecab',
    '  run_host_arch "$BREW_BIN" list python@3.11 >/dev/null 2>&1 || run_host_arch "$BREW_BIN" install python@3.11',
    '  run_host_arch "$BREW_BIN" list unzip >/dev/null 2>&1 || run_host_arch "$BREW_BIN" install unzip',
    '  if [ -n "$BREW_BIN" ]; then eval "$(run_host_arch "$BREW_BIN" shellenv)"; fi',
    'fi',
    ...
    '  if [ -z "$CANDIDATE_PATH" ]; then continue; fi',
    '  CANDIDATE_ARCH="$(run_host_arch "$CANDIDATE_PATH" -c \'import platform; print(platform.machine())\' 2>/dev/null || true)"',
    '  if [ "$HOST_ARCH" = "arm64" ] && [ "$CANDIDATE_ARCH" != "arm64" ] && [ "$CANDIDATE_ARCH" != "aarch64" ]; then continue; fi',
    '  PYTHON_BIN="$CANDIDATE_PATH"',
    '  break',
    'done',
    'if [ -z "$PYTHON_BIN" ] && [ "$HOST_ARCH" = "arm64" ]; then echo "未找到 arm64 Python 3.11。请先执行：/opt/homebrew/bin/brew install python@3.11"; exit 1; fi',
    'if [ -z "$PYTHON_BIN" ]; then echo "未找到 Python 3.11。请先执行：brew install python@3.11"; exit 1; fi',
    'PYTHON_MACHINE="$(run_host_arch "$PYTHON_BIN" -c \'import platform; print(platform.machine())\' 2>/dev/null || true)"',
    'PY_VERSION="$(run_host_arch "$PYTHON_BIN" -V 2>&1 | awk \'{print $2}\' | cut -d. -f1,2)"',
    ...
    '  VENV_VERSION="$(run_host_arch .venv/bin/python -V 2>&1 | awk \'{print $2}\' | cut -d. -f1,2)"',
    ...
    'fi',
    'if [ ! -d .venv ]; then',
    '  run_host_arch "$PYTHON_BIN" -m venv .venv',
    'fi',
    'VENV_PYTHON=".venv/bin/python"',
    'if [ ! -x "$VENV_PYTHON" ]; then echo ".venv 创建失败：未找到 $VENV_PYTHON"; exit 1; fi',
    'run_host_arch "$VENV_PYTHON" -V',
    'run_host_arch "$VENV_PYTHON" -m pip install --upgrade pip wheel setuptools',
    ...
    '    fi',
    '  fi',
    '  run_host_arch "$VENV_PYTHON" -m pip install -r "$INSTALL_REQUIREMENTS_FILE"',
    '  if [ "$INSTALL_REQUIREMENTS_FILE" != "requirements.txt" ]; then rm -f "$INSTALL_REQUIREMENTS_FILE"; fi',
    'else',
    '  echo "未找到 requirements.txt，跳过依赖安装。"',
    'fi',
    'run_host_arch "$VENV_PYTHON" -m pip install torchcodec',
    'run_host_arch "$VENV_PYTHON" -c "import torchcodec"',
